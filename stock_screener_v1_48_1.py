...
# ============================================================
# A股短线研究工具 V1.48.1：MACD修复 + 市场状态 + 当前强弱 + 稳健优选排序
#
# V1.2 目标：
...
DEFAULT_PARAMS = {
    ...
    # V1.48：防守韧性不再单独决定去留；历史防守只做背景因子，当前行情行为决定是否允许进入最终池。
    'extreme_pick_count': 5,
    # V1.48 极度推荐：对应当前行情下的稳健启动框架，而不是单一指标排名。
    ...
    'macd_stage_hard_gate': True,
    'macd_min_green_shrink_days': 2,
    'macd_min_gap_shrink_2d_pct': 4.0,
    'macd_sideways_max_return_10d': 2.5,
    'macd_sideways_max_range_10d': 6.0,

    # V1.48：历史防守是“背景分”，不再作为单独硬淘汰条件。
    'defensive_enabled': True,
    'defensive_min_history_days': 60,
    'defensive_min_bad_days': 8,
    'defensive_bad_day_threshold': -0.8,
    'defensive_hold_threshold': -0.5,
    'defensive_weight': 25.0,
    'defensive_extreme_weight': 20.0,
    'defensive_strict_weight': 15.0,

    # V1.48：当前行情风险闸门。越弱的市场要求越高的“当前相对强度”。
    'regime_enabled': True,
    'regime_weak_score': 45.0,
    'regime_very_weak_score': 35.0,
    'current_min_score_normal': 55.0,
    'current_min_score_weak': 66.0,
    'current_min_score_very_weak': 76.0,
    'current_min_rel5_weak': 1.0,
    'current_min_rel20_weak': 0.0,
    'current_min_rel5_very_weak': 2.0,
    'current_min_rel20_very_weak': 1.0,
    'current_max_today_gain_weak': 2.5,
    'current_max_today_gain_very_weak': 2.0,
    'current_max_ret20_weak': 12.0,
    'current_max_ret20_very_weak': 10.0,
    'current_max_dd10_weak': -6.0,
    'current_max_dd10_very_weak': -5.0,
    'current_max_dd60_weak': -22.0,
    'current_max_dd60_very_weak': -18.0,
    'current_max_down_shock_weak': -5.0,
    'current_max_down_shock_very_weak': -4.0,
    'current_max_down3_count_weak': 2,
    'current_max_down3_count_very_weak': 1,

    # V1.48：弱市采用“防守型分流”，不再强制所有股票都满足MACD死叉修复；强市/震荡仍保留原修复策略。
    'weak_allow_defensive_track': True,
    'defensive_track_min_score_weak': 67.0,
    'defensive_track_min_score_very_weak': 76.0,
    'defensive_track_max_today_gain_weak': 2.5,
    'defensive_track_max_today_gain_very_weak': 2.0,
    'defensive_track_max_ret20_weak': 10.0,
    'defensive_track_max_ret20_very_weak': 8.0,
    'defensive_track_max_dd60_weak': -20.0,
    'defensive_track_max_dd60_very_weak': -16.0,
    'defensive_track_min_rel5_weak': 1.0,
    'defensive_track_min_rel5_very_weak': 2.0,
    'defensive_track_min_rel20_weak': 0.0,
    'defensive_track_min_rel20_very_weak': 1.0,
    'defensive_track_max_down3_weak': 1,
    'defensive_track_max_down3_very_weak': 1,

    # V1.48：最终关注池上限进一步收敛；没有合格股票时宁可0只。

    # 最终关注池上限：避免强市几十只、弱市也几十只。无合格标的时允许0只。
    'regime_max_picks_strong': 12,
    'regime_max_picks_normal': 8,
    'regime_max_picks_weak': 5,
    'regime_max_picks_very_weak': 3,
}
...
def price_position(raw_df, days=120):
    if raw_df is None or len(raw_df) < days:
        return np.nan
    close = safe_float(raw_df['收盘'].iloc[-1])
    low = safe_float(pd.to_numeric(raw_df['最低'].tail(days), errors='coerce').min())
    high = safe_float(pd.to_numeric(raw_df['最高'].tail(days), errors='coerce').max())
...
def check_price(raw_df, min_price, max_price):
    if raw_df is None or raw_df.empty:
        return False
    price = safe_float(raw_df.iloc[-1].get('收盘') if hasattr(raw_df.iloc[-1], 'get') else None)
    if price is None:
        return False
...
def macd_stage_state(qfq_df, tangle_ratio=0.35):
    """V1.48：显式识别早期死叉修复、缠绕横盘、弱修复和前期大涨回撤。"""
    out = {
        'score': 0.0, 'state': '数据不足',
        'green_2d': False, 'green_3d': False, 'green_shrink_days': 0,
        'hist_negative': False, 'death_cross': False,
        'gap': np.nan, 'gap_ratio': np.nan,
        'gap_shrink_1d_pct': np.nan, 'gap_shrink_2d_pct': np.nan,
        'ma5_above': False, 'ma5_stable': False, 'ma5_rising': False,
        'ma5_score': 50.0, 'ma5_dist_pct': np.nan,
        'sideways_10d': False, 'ret10': np.nan, 'range10_pct': np.nan, 'pos20': np.nan,
        'repair_quality': 0.0,
        'pullback_state': '正常修复', 'ret60': np.nan,
        'drawdown60_pct': np.nan, 'max20d_gain60': np.nan,
        'reason': ''
    }
    if qfq_df is None or qfq_df.empty or len(qfq_df) < 30:
        return out
    try:
        close = pd.to_numeric(qfq_df['收盘'], errors='coerce')
        dif, dea, hist = calculate_macd(qfq_df)
        ...
        if len(h) < 6 or len(d) < 6 or len(e) < 6:
            return out

        h0, h1, h2, h3 = map(float, [h.iloc[-1], h.iloc[-2], h.iloc[-3], h.iloc[-4]])
        d0, d1, d2 = map(float, [d.iloc[-1], d.iloc[-2], d.iloc[-3]])
        e0, e1, e2 = map(float, [e.iloc[-1], e.iloc[-2], e.iloc[-3]])
        negative = bool(h0 < 0)
        death_cross = bool(d0 < e0)
        ...
        if negative:
            ...
            for i in range(len(h) - 2, -1, -1):
                ...
                if cur < 0 and prev > cur:
                    ...
                    break
        green_days = int(min(green_days, 10))
        ...
        gap_ratio = gap0 / med20 if pd.notna(med20) and med20 > 0 else np.nan
        shrink1 = ((gap1 - gap0) / gap1 * 100.0) if gap1 > 1e-12 else np.nan
        shrink2 = ((gap2 - gap0) / gap2 * 100.0) if gap2 > 1e-12 else np.nan

        ma5 = close.rolling(5).mean()
        c0, c1 = float(close.iloc[-1]), float(close.iloc[-2])
        m0, m1 = float(ma5.iloc[-1]), float(ma5.iloc[-2])
        ma5_above = bool(c0 >= m0)
        ma5_rising = bool(m0 > m1)
        ma5_stable = bool(ma5_above and c1 >= m1)
        ma_dist = ((c0 / m0) - 1) * 100 if m0 else 0.0
        if ma5_stable and ma5_rising: ma5_score = 100.0
        elif ma5_above and ma5_rising: ma5_score = 90.0
        elif ma5_above: ma5_score = 78.0
        elif ma_dist >= -0.8 and ma5_rising: ma5_score = 62.0
        elif ma_dist >= -1.5: ma5_score = 42.0
        else: ma5_score = 25.0

        ret10 = np.nan; range10 = np.nan; pos20 = np.nan
        if len(close) > 10 and close.iloc[-11] > 0:
            ret10 = (c0 / float(close.iloc[-11]) - 1) * 100
            hi10 = float(pd.to_numeric(qfq_df['最高'], errors='coerce').tail(10).max())
            lo10 = float(pd.to_numeric(qfq_df['最低'], errors='coerce').tail(10).min())
            if lo10 > 0:
                range10 = (hi10 / lo10 - 1) * 100
                pos20 = (c0 - lo10) / (hi10 - lo10) if hi10 > lo10 else 0.5
        sideways = bool(
            pd.notna(ret10) and pd.notna(range10) and
            abs(ret10) <= 2.5 and range10 <= 6.0
        )

        ret60 = np.nan; dd60 = np.nan; max20_gain60 = np.nan
        pullback_state = '正常修复'
        try:
            rc = close.dropna().reset_index(drop=True)
            if len(rc) > 60 and rc.iloc[-61] > 0:
                ret60 = (c0 / float(rc.iloc[-61]) - 1) * 100
            high60 = float(pd.to_numeric(qfq_df['最高'], errors='coerce').tail(60).max())
            if high60 > 0: dd60 = (c0 / high60 - 1) * 100
            rr = rc.tail(min(60, len(rc)))
            if len(rr) >= 22:
                max20_gain60 = float((rr / rr.shift(20) - 1).max() * 100)
            if pd.notna(max20_gain60) and pd.notna(dd60) and max20_gain60 >= 25 and dd60 <= -12:
                pullback_state = '前期大涨后深回撤'
            elif pd.notna(ret60) and pd.notna(dd60) and ret60 >= 15 and dd60 <= -8:
                pullback_state = '前期上涨后回撤'
        except Exception:
            pass

        score = 18.0 if death_cross and negative else 5.0
        reasons = ['仍处死叉' if death_cross else '已脱离死叉']
        if green3:
            score += 34; reasons.append('绿柱连续3日缩短')
        elif green2:
            score += 24; reasons.append('绿柱连续2日缩短')
        elif negative and h0 > h1:
            score += 10; reasons.append('绿柱开始缩短')
        else:
            score -= 12
        if pd.notna(shrink2):
            if shrink2 >= 15: score += 18; reasons.append(f'MACD距两日缩小{shrink2:.1f}%')
            elif shrink2 >= 8: score += 12; reasons.append(f'MACD距两日缩小{shrink2:.1f}%')
            elif shrink2 >= 4: score += 7
            elif shrink2 < 0: score -= 12; reasons.append('MACD距重新扩大')
        if d0 > d1: score += 8; reasons.append('DIF向上')
        if ma5_stable and ma5_rising: score += 8; reasons.append('股价站稳5日线')
        elif ma5_above and ma5_rising: score += 5; reasons.append('股价站上5日线')

        tangle = bool(pd.notna(gap_ratio) and gap_ratio < tangle_ratio)
        if tangle:
            score -= 28; reasons.append('快慢线已进入缠绕区')
        elif pd.notna(gap_ratio) and gap_ratio < 0.55:
            score -= 6; reasons.append('接近缠线区')
        elif pd.notna(gap_ratio) and gap_ratio <= 1.10:
            score += 5
        if sideways:
            score -= 16; reasons.append('价格近10日横盘')
        if pd.notna(ret10) and ret10 <= -5 and not ma5_above:
            score -= 10; reasons.append('价格修复弱于MACD')

        repair_quality = float(np.clip(score + 10.0, 0, 100))
        state = (
            '缠绕横盘' if tangle and sideways else
            '临界缠绕' if tangle else
            '早期修复' if green3 and death_cross else
            '修复中' if green2 and death_cross else
            '弱修复' if death_cross and (green_days >= 1 or (pd.notna(shrink1) and shrink1 > 0)) else
            '修复待确认'
        )
        out.update({
            'score': float(np.clip(score, 0, 100)), 'state': state,
            'green_2d': green2, 'green_3d': green3, 'green_shrink_days': green_days,
            'hist_negative': negative, 'death_cross': death_cross,
            'gap': float(gap0), 'gap_ratio': float(gap_ratio) if pd.notna(gap_ratio) else np.nan,
            'gap_shrink_1d_pct': float(shrink1) if pd.notna(shrink1) else np.nan,
            'gap_shrink_2d_pct': float(shrink2) if pd.notna(shrink2) else np.nan,
            'ma5_above': ma5_above, 'ma5_stable': ma5_stable, 'ma5_rising': ma5_rising,
            'ma5_score': float(ma5_score), 'ma5_dist_pct': float(ma_dist),
            'sideways_10d': sideways, 'pos20': float(pos20) if pd.notna(pos20) else np.nan,
            'ret10': float(ret10) if pd.notna(ret10) else np.nan,
            'range10_pct': float(range10) if pd.notna(range10) else np.nan,
            'repair_quality': repair_quality,
            'pullback_state': pullback_state,
            'ret60': float(ret60) if pd.notna(ret60) else np.nan,
            'drawdown60_pct': float(dd60) if pd.notna(dd60) else np.nan,
            'max20d_gain60': float(max20_gain60) if pd.notna(max20_gain60) else np.nan,
...
def macd_stage_gate(qfq_df, params):
    """V1.48：主候选只保留早期修复窗口，明确排除缠绕横盘。"""
    stage = macd_stage_state(qfq_df, float(params.get('macd_tangle_ratio', 0.35)))
    if not params.get('macd_stage_hard_gate', True):
        return True, stage, '未启用阶段硬门槛'
    if not stage.get('death_cross') or not stage.get('hist_negative'):
        return False, stage, '当前不是负值死叉修复'
    if int(stage.get('green_shrink_days', 0)) < int(params.get('macd_min_green_shrink_days', 2) or 2):
        return False, stage, '绿柱连续缩短不足'
    shrink2 = safe_float(stage.get('gap_shrink_2d_pct'))
    if shrink2 is not None and shrink2 < float(params.get('macd_min_gap_shrink_2d_pct', 4.0)):
        return False, stage, 'MACD距两日收敛不足'
    if stage.get('state') in ('临界缠绕', '缠绕横盘'):
        return False, stage, '快慢线已缠绕，不属于早期修复窗口'
    sideways_ret = safe_float(stage.get('ret10'))
    sideways_range = safe_float(stage.get('range10_pct'))
    max_ret = float(params.get('macd_sideways_max_return_10d', 2.5) or 2.5)
    max_range = float(params.get('macd_sideways_max_range_10d', 6.0) or 6.0)
    is_sideways = bool(sideways_ret is not None and sideways_range is not None and abs(sideways_ret) <= max_ret and sideways_range <= max_range)
    if is_sideways:
        if shrink2 is None or shrink2 < 8.0:
            return False, stage, '价格近10日横盘且MACD修复力度不足'
    ret10 = safe_float(stage.get('ret10')); ma_dist = safe_float(stage.get('ma5_dist_pct'))
    if ret10 is not None and ma_dist is not None and ret10 <= -5 and ma_dist < -1.5:
        return False, stage, '价格弱于MACD修复'
    return True, stage, stage.get('reason', '有效修复')


def turnover_rate_from_profile(raw_df, profile):
    """V1.48：用成交额/流通市值计算近似换手率，避免依赖额外接口。"""
    try:
...
def sector_context_map(raw_dict, qfq_dict, profiles):
    """V1.48：基于同日全市场股票画像+日线，给候选补充所属板块环境。"""
    groups = {}
...
def macd_repair_entry_quality(raw_df, qfq_df, market_ctx=None):
    """V1.48：入场质量强调绿柱收缩与非缠绕窗口。"""
    mr = macd_repair_state(qfq_df)
    stage = macd_stage_state(qfq_df)
    ps = signal_day_price_state(raw_df)
    score = 0.0; reasons = []
    gd = int(stage.get('green_shrink_days', 0))
    if gd >= 3: score += 30; reasons.append('绿柱连续3日以上缩短')
    elif gd >= 2: score += 24; reasons.append('绿柱连续2日缩短')
    elif gd == 1: score += 8
    if mr.get('repair_2d'): score += 18; reasons.append('死叉距连续两天改善')
    elif mr.get('repair_1d'): score += 7
    shrink2 = safe_float(stage.get('gap_shrink_2d_pct'))
    if shrink2 is not None:
        if shrink2 >= 10: score += 15
        elif shrink2 >= 4: score += 8
        elif shrink2 < 0: score -= 12; reasons.append('MACD距重新扩大')
    if stage.get('state') in ('临界缠绕','缠绕横盘'):
        score -= 35; reasons.append('快慢线缠绕/横盘')
    if stage.get('sideways_10d'):
        score -= 15; reasons.append('价格近10日横盘')
    if stage.get('ma5_stable'):
        score += 10; reasons.append('股价站稳5日线')
    elif stage.get('ma5_above'):
        score += 6; reasons.append('股价站上5日线')
    pct = ps.get('pct')
    if pct is not None and not pd.isna(pct):
        if -1.0 <= pct <= 1.5: score += 10; reasons.append('推荐日未明显追涨')
        elif 1.5 < pct <= 2.5: score += 3
        elif 2.5 < pct < 4: score -= 7; reasons.append('推荐日上涨偏快')
        elif pct >= 4: score -= 25; reasons.append('推荐日上涨过快')
        elif pct <= -3: score -= 10
    con = volume_contraction(raw_df); vr = calculate_volume_ratio(raw_df)
    if not pd.isna(con):
        if 0.55 <= con <= 0.85: score += 10; reasons.append('前期缩量')
        elif con <= 0.95: score += 5
        elif con > 1.10: score -= 5
    if vr is not None:
        if 1.0 <= vr <= 1.6: score += 7
        elif 1.6 < vr <= 2: score += 2
        elif vr > 2.2: score -= 5
        elif vr < 0.6: score -= 6
    try:
        rs = calculate_rsi(qfq_df)
        if len(rs)>=2 and pd.notna(rs.iloc[-1]) and pd.notna(rs.iloc[-2]) and rs.iloc[-1] > rs.iloc[-2]: score += 5; reasons.append('RSI回升')
    except Exception: pass
    try:
        ks,ds,js = calculate_kdj(qfq_df)
        if len(ks)>=2 and pd.notna(ks.iloc[-1]) and pd.notna(ks.iloc[-2]) and ks.iloc[-1] > ks.iloc[-2]: score += 3
        if len(ds)>=2 and pd.notna(ds.iloc[-1]) and pd.notna(ds.iloc[-2]) and ds.iloc[-1] > ds.iloc[-2]: score += 3
    except Exception: pass
...
def limit_up_gene_state(raw_df, threshold=9.8, lookback=LOOKBACK_TRADING_DAYS):
    """V1.48：把涨停基因从“涨停次数”升级为历史弹性、近期密度、距离最近涨停
    以及历史涨停后3个交易日的延续能力。
...
def _market_daily_series(raw_dict):
    """V1.48：构造目标日及以前的全市场日度中位涨跌、上涨占比等序列。"""
    rows = []
    for code, df in (raw_dict or {}).items():
        if df is None or df.empty or '日期' not in df.columns:
            continue
        try:
            tmp = df[['日期', '收盘']].copy()
            tmp['日期'] = pd.to_datetime(tmp['日期'], errors='coerce')
            tmp['收盘'] = pd.to_numeric(tmp['收盘'], errors='coerce')
            tmp = tmp.dropna(subset=['日期', '收盘']).sort_values('日期')
            if len(tmp) < 2:
                continue
            if '涨跌幅' in df.columns:
                pct = pd.to_numeric(df.loc[tmp.index, '涨跌幅'], errors='coerce')
                pct = pct.where(pct.notna(), tmp['收盘'].pct_change() * 100.0)
            else:
                pct = tmp['收盘'].pct_change() * 100.0
            out = pd.DataFrame({'日期': tmp['日期'].values, 'pct': pct.values})
            ...
            continue
    if not rows:
        return pd.DataFrame(columns=['日期', 'market_median_pct', 'advance_ratio', 'sample_size'])
    all_ret = pd.concat(rows, ignore_index=True)
    g = all_ret.groupby('日期')['pct']
    out = g.agg(
        market_median_pct='median',
        advance_ratio=lambda s: float((s > 0).mean()),
        sample_size='count',
    ).reset_index()
    return out.sort_values('日期').reset_index(drop=True)


def market_regime(market_ctx=None, params=None):
    """V1.48：根据当日市场评分、上涨占比和市场中位数定义环境。"""
    params = params or {}
    ctx = market_ctx or {}
    if not params.get('regime_enabled', True):
        return '常态市'
    score = safe_float(ctx.get('score'))
    adv = safe_float(ctx.get('advance_ratio'))
    med = safe_float(ctx.get('median_pct'))
    score = 50.0 if score is None else score
    if score < float(params.get('regime_very_weak_score', 35.0)) or (score < 40 and adv is not None and adv < 0.36) or (med is not None and med <= -1.5 and adv is not None and adv < 0.42):
        return '极弱市'
    if score < float(params.get('regime_weak_score', 45.0)) or (adv is not None and adv < 0.42 and score < 50):
        return '弱市'
    if score >= 60 and (adv is None or adv >= 0.52):
        return '强市'
    return '震荡市'


def regime_pick_limit(market_ctx=None, params=None):
    """V1.48：按市场环境控制最终关注池数量。"""
    params = params or {}
    regime = market_regime(market_ctx, params)
    mapping = {
        '强市': int(params.get('regime_max_picks_strong', 12)),
        '震荡市': int(params.get('regime_max_picks_normal', 8)),
        '弱市': int(params.get('regime_max_picks_weak', 3)),
        '极弱市': int(params.get('regime_max_picks_very_weak', 2)),
    }
    return regime, max(0, mapping.get(regime, 8))


def calculate_current_resilience(raw_df, qfq_df, market_ctx=None, defensive=None, params=None):
    """V1.48：当前行情强弱与“下跌损伤”评分。

    这层回答的是“今天如果市场转弱，这只股票当前状态是否仍然健康”，
    与历史防守韧性分开，防止长期抗跌特征掩盖近期已经走弱的股票。
    """
    params = params or {}
    empty = {
        'score': 50.0, 'state': '数据不足',
        'rel1': np.nan, 'rel5': np.nan, 'rel20': np.nan,
        'ret5': np.nan, 'ret10': np.nan, 'ret20': np.nan,
        'ma5': np.nan, 'ma10': np.nan, 'ma20': np.nan, 'ma60': np.nan,
        'dd10': np.nan, 'dd60': np.nan, 'down_shock10': np.nan,
        'down3_count10': 0, 'today_pct': np.nan, 'close_vs_ma20': np.nan,
        'reason': '数据不足'
    }
    if raw_df is None or qfq_df is None or raw_df.empty or qfq_df.empty or len(raw_df) < 30 or len(qfq_df) < 30:
        return empty
    try:
        raw = raw_df.copy().sort_values('日期').reset_index(drop=True)
        q = qfq_df.copy().sort_values('日期').reset_index(drop=True)
        raw['收盘'] = pd.to_numeric(raw['收盘'], errors='coerce')
        q['收盘'] = pd.to_numeric(q['收盘'], errors='coerce')
        if '涨跌幅' in raw.columns:
            pct = pd.to_numeric(raw['涨跌幅'], errors='coerce')
        else:
            pct = raw['收盘'].pct_change() * 100.0
        pct = pct.fillna(raw['收盘'].pct_change() * 100.0)
        close = raw['收盘']
        qclose = q['收盘']
        def ret(n):
            return float((close.iloc[-1] / close.iloc[-1-n] - 1.0) * 100.0) if len(close) > n and close.iloc[-1-n] else np.nan
        rel = _relative_strength(raw_df, market_ctx or {})
        rel1, rel5, rel20 = rel.get('rel1'), rel.get('rel5'), rel.get('rel20')
        ret5, ret10, ret20 = ret(5), ret(10), ret(20)
        ma5 = float(qclose.rolling(5).mean().iloc[-1])
        ma10 = float(qclose.rolling(10).mean().iloc[-1])
        ma20 = float(qclose.rolling(20).mean().iloc[-1])
        ma60 = float(qclose.rolling(60).mean().iloc[-1]) if len(qclose) >= 60 else np.nan
        dd10_series = qclose.tail(10)
        dd10 = float(((dd10_series / dd10_series.cummax()) - 1.0).min() * 100.0) if len(dd10_series) >= 5 else np.nan
        dd60_series = qclose.tail(60)
        dd60 = float(((dd60_series / dd60_series.cummax()) - 1.0).min() * 100.0) if len(dd60_series) >= 20 else np.nan
        down_shock10 = float(pct.tail(10).min()) if pct.tail(10).notna().any() else np.nan
        down3_count10 = int((pct.tail(10) <= -3.0).sum())
        today_pct = safe_float(pct.iloc[-1])
        close_vs_ma20 = float((qclose.iloc[-1] / ma20 - 1.0) * 100.0) if ma20 else np.nan

        score = 50.0
        reasons=[]
        # 当前相对强度：弱市下最重要
        if rel5 is not None:
            score += float(np.clip(rel5, -6, 6)) * 2.2
            if rel5 >= 2: reasons.append('5日明显跑赢市场')
            elif rel5 < -1: reasons.append('5日相对市场走弱')
        if rel20 is not None:
            score += float(np.clip(rel20, -12, 12)) * 0.85
            if rel20 >= 2: reasons.append('20日相对强度良好')
            elif rel20 < 0: reasons.append('20日相对强度不足')
        # 趋势健康
        if qclose.iloc[-1] > ma20: score += 9; reasons.append('收盘站上20日线')
        else: score -= 9; reasons.append('收盘跌破20日线')
        if pd.notna(ma60) and ma20 > ma60: score += 7; reasons.append('20日线高于60日线')
        elif pd.notna(ma60): score -= 6
        if qclose.iloc[-1] >= ma5 >= ma10: score += 5
        # 近期实际损伤
        if ret5 >= 0: score += 5
        elif ret5 < -3: score -= 6
        if ret20 > 12: score -= 8; reasons.append('20日涨幅偏快')
        elif -2 <= ret20 <= 10: score += 4
        if pd.notna(dd10):
            if dd10 >= -3: score += 7
            elif dd10 >= -6: score += 2
            elif dd10 < -8: score -= 10; reasons.append('近10日回撤偏大')
        if pd.notna(dd60):
            if dd60 >= -12: score += 6
            elif dd60 < -22: score -= 8; reasons.append('60日回撤偏大')
        if pd.notna(down_shock10):
            if down_shock10 <= -5: score -= 10; reasons.append('近期出现明显下杀')
            elif down_shock10 <= -3: score -= 5
            else: score += 3
        score -= min(12, down3_count10 * 3.0)
        # 今日追高惩罚：弱市中尤其重要
        if today_pct is not None:
            if today_pct > 4: score -= 12; reasons.append('推荐日涨幅过快')
            elif today_pct > 2.5: score -= 6; reasons.append('推荐日偏强，防追高')
            elif today_pct < -3: score -= 8; reasons.append('推荐日下跌过快')
        # 历史防守只做小幅背景加权，避免盖住近期恶化
        hs = safe_float((defensive or {}).get('score'))
        if hs is not None: score += np.clip(hs - 50.0, -15, 15) * 0.18
        score = float(np.clip(score, 0, 100))
        if score >= 75: state='当前强势且损伤较低'
        elif score >= 65: state='当前相对稳健'
        elif score >= 55: state='当前中性'
        elif score >= 45: state='当前偏弱'
        else: state='当前风险偏高'
        return {
            ...
            'dd10':dd10, 'dd60':dd60, 'down_shock10':down_shock10,
            'down3_count10':down3_count10, 'today_pct':today_pct,
            'close_vs_ma20':close_vs_ma20,
            'reason':'；'.join(reasons[:8]) or '当前结构中性'
        }
    except Exception:
        return empty


def current_resilience_gate(current, market_ctx=None, params=None):
    """V1.48：当前行情闸门；弱市宁可0只，也不强行留下“看起来最不差”的股票。"""
    params = params or {}
    c = current or {}
    score = safe_float(c.get('score'))
    if score is None: return False, '当前强弱数据不足'
    regime, _ = regime_pick_limit(market_ctx, params)
    if regime == '极弱市':
        min_score=float(params.get('current_min_score_very_weak',76.0)); min_rel5=float(params.get('current_min_rel5_very_weak',2.0)); min_rel20=float(params.get('current_min_rel20_very_weak',1.0)); max_today=float(params.get('current_max_today_gain_very_weak',2.0)); max_ret20=float(params.get('current_max_ret20_very_weak',10.0)); max_dd10=float(params.get('current_max_dd10_very_weak',-5.0)); max_dd60=float(params.get('current_max_dd60_very_weak',-18.0)); max_shock=float(params.get('current_max_down_shock_very_weak',-4.0)); max_down3=int(params.get('current_max_down3_count_very_weak',1))
        level='极弱市'
    elif regime == '弱市':
        min_score=float(params.get('current_min_score_weak',66.0)); min_rel5=float(params.get('current_min_rel5_weak',1.0)); min_rel20=float(params.get('current_min_rel20_weak',0.0)); max_today=float(params.get('current_max_today_gain_weak',2.5)); max_ret20=float(params.get('current_max_ret20_weak',12.0)); max_dd10=float(params.get('current_max_dd10_weak',-6.0)); max_dd60=float(params.get('current_max_dd60_weak',-22.0)); max_shock=float(params.get('current_max_down_shock_weak',-5.0)); max_down3=int(params.get('current_max_down3_count_weak',2))
        level='弱市'
    else:
        min_score=float(params.get('current_min_score_normal',55.0)); min_rel5=-1.0; min_rel20=-3.0; max_today=4.0; max_ret20=18.0; max_dd10=-10.0; max_dd60=-30.0; max_shock=-6.0; max_down3=3
        level='常态'
    if score < min_score: return False, f'{level}当前强弱分{score:.1f}<{min_score:.1f}'
    for key,minimum,label in [('rel5',min_rel5,'5日相对强度'),('rel20',min_rel20,'20日相对强度')]:
        v=safe_float(c.get(key))
        if v is None or v < minimum: return False, f'{level}{label}不足'
    ma20=safe_float(c.get('ma20')); close_ma20=safe_float(c.get('close_vs_ma20'))
    if regime in ('弱市','极弱市') and (ma20 is None or close_ma20 is None or close_ma20 < 0): return False, f'{level}未站稳20日线'
    today=safe_float(c.get('today_pct'))
    if today is not None and today > max_today: return False, f'{level}推荐日涨幅{today:.2f}%偏快'
    ret20=safe_float(c.get('ret20'))
    if ret20 is not None and ret20 > max_ret20: return False, f'{level}20日涨幅过快'
    dd10=safe_float(c.get('dd10'))
    if dd10 is not None and dd10 < max_dd10: return False, f'{level}近10日回撤过大'
    dd60=safe_float(c.get('dd60'))
    if dd60 is not None and dd60 < max_dd60: return False, f'{level}60日回撤过大'
    shock=safe_float(c.get('down_shock10'))
    if shock is not None and shock < max_shock: return False, f'{level}近期出现过大下杀'
    if int(c.get('down3_count10') or 0) > max_down3: return False, f'{level}近10日大跌次数过多'
    return True, f'{level}当前强弱通过'


def calculate_defensive_resilience(raw_df, qfq_df=None, market_ctx=None, params=None):
    """V1.48：计算股票在弱市环境下的历史韧性。

    核心不是预测“不会跌”，而是回答：过去市场明显偏弱时，它是否比大多数股票更抗跌，
    同时自身中期趋势/回撤是否健康。所有计算严格使用目标日及以前数据。
    """
    params = params or {}
    empty = {
        'score': 50.0,
        'state': '数据不足',
        'bad_days': 0,
        'outperform_ratio': np.nan,
        'hold_ratio': np.nan,
        'bad_day_rel_edge': np.nan,
        'rel20': np.nan,
        'rel60': np.nan,
        'vol20': np.nan,
    ...
    if raw_df is None or raw_df.empty:
        return empty
    try:
        raw = raw_df[['日期', '收盘']].copy()
        raw['日期'] = pd.to_datetime(raw['日期'], errors='coerce')
        raw['收盘'] = pd.to_numeric(raw['收盘'], errors='coerce')
        raw = raw.dropna(subset=['日期', '收盘']).sort_values('日期').drop_duplicates('日期', keep='last')
        if len(raw) < int(params.get('defensive_min_history_days', 60)):
            empty['valid_days'] = len(raw)
            return empty

        if '涨跌幅' in raw_df.columns:
            p = raw_df[['日期', '涨跌幅']].copy()
            p['日期'] = pd.to_datetime(p['日期'], errors='coerce')
            p['pct'] = pd.to_numeric(p['涨跌幅'], errors='coerce')
            p = p.dropna(subset=['日期']).drop_duplicates('日期', keep='last')
            raw = raw.merge(p[['日期', 'pct']], on='日期', how='left')
        else:
            raw['pct'] = np.nan
        raw['pct'] = raw['pct'].fillna(raw['收盘'].pct_change() * 100.0)

        mctx = market_ctx or {}
        mseries = mctx.get('daily_series')
        if not isinstance(mseries, pd.DataFrame) or mseries.empty:
            # 单股票调用场景没有预构建序列时，不能凭股票自身制造“市场”，因此返回中性。
            return empty
        mseries = mseries.copy()
        mseries['日期'] = pd.to_datetime(mseries['日期'], errors='coerce')
        merged = raw.merge(mseries, on='日期', how='inner')
        merged = merged.dropna(subset=['pct', 'market_median_pct']).sort_values('日期')
        if len(merged) < 30:
            empty['valid_days'] = len(merged)
            return empty

        bad_threshold = float(params.get('defensive_bad_day_threshold', -0.8))
        bad = merged[
            (merged['market_median_pct'] <= bad_threshold) |
            ((merged['market_median_pct'] < 0) & (merged['advance_ratio'] <= 0.42))
        ].copy()
        # 若严格弱市日太少，退一步使用所有市场中位数<0的交易日，避免小样本失真。
        if len(bad) < int(params.get('defensive_min_bad_days', 8)):
            bad = merged[merged['market_median_pct'] < 0].copy()
        if len(bad) < int(params.get('defensive_min_bad_days', 8)):
            empty['valid_days'] = len(merged)
            empty['bad_days'] = len(bad)
            return empty

        stock_bad = pd.to_numeric(bad['pct'], errors='coerce')
        market_bad = pd.to_numeric(bad['market_median_pct'], errors='coerce')
        rel_daily = stock_bad - market_bad
        outperf_ratio = float((rel_daily >= 0).mean() * 100.0)
        hold_threshold = float(params.get('defensive_hold_threshold', -0.5))
        hold_ratio = float((stock_bad >= hold_threshold).mean() * 100.0)
        bad_edge = float(rel_daily.median())

        # 20/60日使用前复权价格，减少分红/除权对中期相对表现的干扰。
        price = raw[['日期', '收盘']].copy()
        if qfq_df is not None and not qfq_df.empty and '日期' in qfq_df.columns and '收盘' in qfq_df.columns:
            q = qfq_df[['日期', '收盘']].copy()
            q['日期'] = pd.to_datetime(q['日期'], errors='coerce')
            q['adj_close'] = pd.to_numeric(q['收盘'], errors='coerce')
            q = q.dropna(subset=['日期', 'adj_close']).drop_duplicates('日期', keep='last')
            price = price[['日期']].merge(q[['日期', 'adj_close']], on='日期', how='left')
            price['adj_close'] = price['adj_close'].fillna(price['adj_close'].ffill())
        else:
            price['adj_close'] = pd.to_numeric(raw['收盘'], errors='coerce').values
        price = price.dropna(subset=['adj_close']).sort_values('日期')

        market_ret = pd.to_numeric(mseries['market_median_pct'], errors='coerce').dropna() / 100.0 + 1.0
        market_ret20 = (float(market_ret.tail(20).prod()) - 1.0) * 100.0 if len(market_ret) >= 20 else np.nan
        market_ret60 = (float(market_ret.tail(60).prod()) - 1.0) * 100.0 if len(market_ret) >= 60 else np.nan
        stock_ret20 = (float(price['adj_close'].iloc[-1] / price['adj_close'].iloc[-21]) - 1.0) * 100.0 if len(price) >= 21 else np.nan
        stock_ret60 = (float(price['adj_close'].iloc[-1] / price['adj_close'].iloc[-61]) - 1.0) * 100.0 if len(price) >= 61 else np.nan
        rel20 = stock_ret20 - market_ret20 if pd.notna(stock_ret20) and pd.notna(market_ret20) else np.nan
        rel60 = stock_ret60 - market_ret60 if pd.notna(stock_ret60) and pd.notna(market_ret60) else np.nan

        daily_vol = raw['pct'].tail(20).std(ddof=1) if len(raw) >= 20 else np.nan
        # 下行Beta：只在市场下跌样本中估算，越低代表市场坏时跟跌程度越小。
        mb = market_bad.to_numpy(dtype=float)
        sb = stock_bad.to_numpy(dtype=float)
        downside_beta = np.nan
        if len(mb) >= 8 and np.var(mb) > 1e-8:
            downside_beta = float(np.cov(sb, mb, ddof=1)[0, 1] / np.var(mb, ddof=1))

        series = price['adj_close'].tail(60).astype(float)
        dd60 = np.nan
        if len(series) >= 20:
            peak = series.cummax()
            dd60 = float(((series / peak) - 1.0).min() * 100.0)

        # 0~100稳定性分：弱市相对表现是核心，中期回撤/下行Beta作为风险约束。
        score = 50.0
        score += np.clip(outperf_ratio - 50.0, -30.0, 30.0) * 0.75
        score += np.clip(hold_ratio - 50.0, -30.0, 30.0) * 0.35
        score += np.clip(bad_edge, -3.0, 3.0) * 5.0
        if pd.notna(rel20): score += np.clip(rel20, -15.0, 15.0) * 0.55
        if pd.notna(rel60): score += np.clip(rel60, -25.0, 25.0) * 0.35
        if pd.notna(downside_beta):
            if downside_beta <= 0.65: score += 9
            elif downside_beta <= 0.85: score += 6
            elif downside_beta <= 1.00: score += 3
            elif downside_beta <= 1.15: score -= 3
            elif downside_beta <= 1.35: score -= 8
            else: score -= 14
        if pd.notna(dd60):
            if dd60 >= -10: score += 8
            elif dd60 >= -15: score += 5
            elif dd60 >= -22: score += 1
            elif dd60 >= -30: score -= 6
            else: score -= 12
        if pd.notna(daily_vol):
            if daily_vol <= 2.0: score += 5
            elif daily_vol <= 2.8: score += 2
            elif daily_vol >= 4.5: score -= 7
        ...
        return {
            ...
            'outperform_ratio': outperf_ratio,
            'hold_ratio': hold_ratio,
            'bad_day_rel_edge': bad_edge,
            'rel20': rel20,
            'rel60': rel60,
            'vol20': float(daily_vol) if pd.notna(daily_vol) else np.nan,
            'downside_beta': downside_beta,
            'drawdown60': dd60,
            'valid_days': int(len(merged)),
        }
    except Exception:
        return empty


def defensive_hard_gate(defensive, market_ctx=None, params=None):
    """V1.48：市场越差，稳定性门槛越高。"""
    params = params or {}
    if not params.get('defensive_enabled', True):
        return True, '防守韧性层关闭'
    d = defensive or {}
    score = safe_float(d.get('score'))
    bad_days = int(d.get('bad_days') or 0)
    valid_days = int(d.get('valid_days') or 0)
    if score is None or valid_days < int(params.get('defensive_min_history_days', 60)) or bad_days < int(params.get('defensive_min_bad_days', 8)):
        return False, '防守数据不足'

    ms = safe_float((market_ctx or {}).get('score'))
    if ms is None: ms = 50.0
    if ms < 35:
        min_score = float(params.get('defensive_very_weak_min_score', 70.0))
        level = '极弱市'
    elif ms < 45:
        min_score = float(params.get('defensive_weak_min_score', 64.0))
        level = '弱市'
    else:
        min_score = float(params.get('defensive_normal_min_score', 58.0))
        level = '常态市'

    if score < min_score:
        return False, f'{level}稳定性分{score:.1f}<{min_score:.1f}'

    outperf = safe_float(d.get('outperform_ratio'))
    rel20 = safe_float(d.get('rel20'))
    rel60 = safe_float(d.get('rel60'))
    dd60 = safe_float(d.get('drawdown60'))
    beta = safe_float(d.get('downside_beta'))
    if ms < 45:
        if outperf is None or outperf < float(params.get('defensive_min_outperform_ratio', 52.0)):
            return False, f'{level}弱市跑赢率不足'
        if rel20 is not None and rel20 < float(params.get('defensive_min_rel20', 0.0)):
            return False, f'{level}相对20日表现转弱'
        if rel60 is not None and rel60 < float(params.get('defensive_min_rel60', 0.0)):
            return False, f'{level}相对60日表现转弱'
    if dd60 is not None and dd60 < float(params.get('defensive_max_drawdown60', -22.0)):
        return False, f'60日最大回撤{dd60:.1f}%过大'
    if beta is not None and beta > float(params.get('defensive_max_downside_beta', 1.15)):
        return False, f'弱市下行Beta{beta:.2f}偏高'
    return True, '防守韧性通过'


def calculate_extreme_score(raw_df, qfq_df, params, market_ctx=None, profile=None, sector_ctx=None, defensive=None):
    """V1.48：极度推荐在V1.48多因子基础上加入防守韧性，但不允许缺失数据凭空加分。"""
    ...
    try:
        rs_info = _relative_strength(raw_df, market_ctx or {})
        rs_score = safe_float(rs_info.get('score')) or 50.0
    except Exception:
        rs_score = 50.0
    parts={}
    parts['macd']=float(np.clip(0.72*stage.get('score',0)+0.28*stage.get('repair_quality',0),0,100))
    parts['ma5']=float(stage.get('ma5_score',50))
    kdj=50.0
    try:
        ks,ds,js=calculate_kdj(qfq_df); k0,d0,j0=float(ks.iloc[-1]),float(ds.iloc[-1]),float(js.iloc[-1]); k1,d1=float(ks.iloc[-2]),float(ds.iloc[-2])
        if k0>d0: kdj+=22
        elif k0<d0: kdj-=8
        if k0>k1: kdj+=12
        if d0>d1: kdj+=8
        if j0>105: kdj-=18
        if j0<20 and k0>d0: kdj+=8
    except Exception: pass
    parts['kdj']=float(np.clip(kdj,0,100))
    rsi_score=50.0
    try:
        rs=calculate_rsi(qfq_df); r0,r1=float(rs.iloc[-1]),float(rs.iloc[-2])
        if 45<=r0<=68: rsi_score+=22
        elif 35<=r0<45: rsi_score+=10
        elif 68<r0<=75: rsi_score+=5
        elif r0>80: rsi_score-=25
        if r0>r1: rsi_score+=18
        else: rsi_score-=4
    except Exception: pass
    parts['rsi']=float(np.clip(rsi_score,0,100))
    vol_score=50.0
    if con is not None and not pd.isna(con):
        if 0.55<=con<=0.85: vol_score+=18
        elif 0.85<con<=1: vol_score+=8
        elif con<0.45: vol_score-=12
        elif con>1.10: vol_score-=8
    if vol is not None and not pd.isna(vol):
        if 1<=vol<=1.6: vol_score+=24
        elif 1.6<vol<=2: vol_score+=7
        elif 0.75<=vol<1: vol_score+=3
        elif vol>2.2: vol_score-=22
        elif vol<0.6: vol_score-=10
    parts['volume']=float(np.clip(vol_score,0,100))
    pos_score=55.0
    pos20 = safe_float(stage.get('pos20'))
    recent_pos_score = 55.0
    if pos20 is not None:
        if 0.15<=pos20<=0.50: recent_pos_score=92.0
        elif 0.05<=pos20<0.15 or 0.50<pos20<=0.68: recent_pos_score=76.0
        elif pos20>0.85: recent_pos_score=35.0
        else: recent_pos_score=58.0
    if pos120 is not None and not pd.isna(pos120):
        if 0.18<=pos120<=0.62: pos_score=86
        elif 0.10<=pos120<0.18 or 0.62<pos120<=0.78: pos_score=70
        elif pos120>0.85: pos_score=35
        else: pos_score=56
    pos_score = 0.65*recent_pos_score + 0.35*pos_score
    if ret20 is not None and not pd.isna(ret20):
        if ret20>20: pos_score-=18
        elif ret20>10: pos_score-=10
        elif -12<=ret20<=5: pos_score+=4
    parts['position']=float(np.clip(pos_score,0,100))
    parts['gene']=float(np.clip(gene.get('score',50),0,100))
    parts['context']=float(np.clip(0.65*market_score+0.35*rs_score,0,100))
    parts['defensive']=float(np.clip((defensive or {}).get('score',50.0),0,100))
    weights={'macd':28.0,'ma5':8.0,'kdj':6.0,'rsi':6.0,'volume':8.0,'position':8.0,'gene':14.0,'context':4.0,'defensive':18.0}
    score=sum(parts[k]*weights[k] for k in weights)/sum(weights.values())
    reasons=[stage.get('state','MACD未知')]
    if stage.get('green_shrink_days',0)>=3: reasons.append(f"绿柱连续{stage.get('green_shrink_days')}日缩短")
    elif stage.get('green_2d'): reasons.append('绿柱连续2日缩短')
    if pd.notna(stage.get('gap_shrink_2d_pct')): reasons.append(f"MACD距两日变化{stage.get('gap_shrink_2d_pct'):.1f}%")
    if stage.get('ma5_stable'): reasons.append('站稳5日线')
    elif stage.get('ma5_above'): reasons.append('站上5日线')
    if con is not None and not pd.isna(con) and vol is not None and 1<=vol<=1.6: reasons.append('缩量后温和放量')
    if stage.get('pullback_state')!='正常修复': reasons.append(stage.get('pullback_state'))
    if stage.get('sideways_10d'): reasons.append('近10日价格偏横盘')
    if pd.notna((defensive or {}).get('score')):
        reasons.append(f"防守韧性{float((defensive or {}).get('score')):.1f}")
    return {'score':round(float(np.clip(score,0,100)),1),'reason':'；'.join(reasons[:9]),'gene':gene,'macd_stage':stage,'turnover':np.nan,'parts':{k:round(v,1) for k,v in parts.items()}}

def apply_extreme_picks(df, top_n=5):
    """V1.48：不减少候选池，只标记极度推荐 Top N。"""
    if df is None or df.empty: return df
    out=df.copy(); out['极度推荐']=''; out['极度推荐排名']=''
    n=max(1,int(top_n or 5))
    sort_cols=[c for c in ['稳健优选分','稳定性分','极度推荐分','涨停基因分','推荐度','严格匹配度'] if c in out.columns]
...
def build_close_trade_plan(raw_df, qfq_df, params):
    ...
    row = raw_df.iloc[-1]
    close = safe_float(row.get('收盘') if hasattr(row, 'get') else None)
    high_today = safe_float(row.get('最高') if hasattr(row, 'get') else None)
    low_today = safe_float(row.get('最低') if hasattr(row, 'get') else None)
    if close is None or high_today is None or low_today is None or close <= 0:
        return {}
    q = qfq_df.copy()
    for c in ['收盘','最高','最低']:
        q[c] = pd.to_numeric(q[c], errors='coerce')
    ma5 = safe_float(q['收盘'].rolling(5).mean().iloc[-1])
    ma10 = safe_float(q['收盘'].rolling(10).mean().iloc[-1])
    ma20 = safe_float(q['收盘'].rolling(20).mean().iloc[-1])
    ma60 = safe_float(q['收盘'].rolling(60).mean().iloc[-1])
    high20 = safe_float(q['最高'].tail(20).max())
    low20 = safe_float(q['最低'].tail(20).min())
...
def build_result(code, name, raw_df, qfq_df, params, market_ctx=None, profile=None, sector_ctx=None):
    ...
    price = safe_float(last_row.get('收盘') if hasattr(last_row, 'get') else None)
    if price is None or price <= 0:
        return None
    ...
    defensive = params.get('_defensive_resilience') or calculate_defensive_resilience(raw_df, qfq_df, market_ctx, params)
    current = params.get('_current_resilience') or calculate_current_resilience(raw_df, qfq_df, market_ctx, defensive, params)
    extreme = calculate_extreme_score(raw_df, qfq_df, params, market_ctx, profile=profile, sector_ctx=sector_ctx, defensive=defensive)
    gene = extreme.get('gene', {})
    stability_score = safe_float(defensive.get('score')) or 50.0
    current_score = safe_float(current.get('score')) or 50.0
    strict_norm = float(np.clip(strict_score, 0, 100))
    extreme_score = float(np.clip(extreme.get('score', 50.0), 0, 100))
    regime_now, _regime_limit = regime_pick_limit(market_ctx, params)
    if regime_now in ('弱市', '极弱市'):
        # 弱市：当前状态是第一优先级；历史防守只能证明“曾经这样”，不能掩盖现在正在变坏。
        current_weight = 55.0
        stable_weight = 15.0
        stable_extreme_weight = 15.0
        stable_strict_weight = 15.0
    else:
        current_weight = 40.0
        stable_weight = float(params.get('defensive_weight', 25.0))
        stable_extreme_weight = float(params.get('defensive_extreme_weight', 20.0))
        stable_strict_weight = float(params.get('defensive_strict_weight', 15.0))
    total_w = max(1.0, current_weight + stable_weight + stable_extreme_weight + stable_strict_weight)
    robust_score = (current_score * current_weight + stability_score * stable_weight + extreme_score * stable_extreme_weight + strict_norm * stable_strict_weight) / total_w
    # V1.48：极度推荐独立计算，不替换原推荐度。
    ...
    macd = macd_state(qfq_df)
    ind = indicator_state(qfq_df)
    stage = extreme.get('macd_stage', {}) or {}
    return {
        '代码': code,
        '名称': name,
        '所属行业': (profile or {}).get('industry','') or '暂无同日行业数据',
        'MACD阶段': stage.get('state',''),
        'MACD距': round(float(stage.get('gap')),4) if pd.notna(stage.get('gap')) else '',
        'MACD距变化1日%': round(float(stage.get('gap_shrink_1d_pct')),1) if pd.notna(stage.get('gap_shrink_1d_pct')) else '',
        'MACD距变化2日%': round(float(stage.get('gap_shrink_2d_pct')),1) if pd.notna(stage.get('gap_shrink_2d_pct')) else '',
        '绿柱连续缩短': int(stage.get('green_shrink_days',0)),
        '修复质量': round(float(stage.get('repair_quality',0)),1),
        '前期形态': stage.get('pullback_state','正常修复'),
        '60日涨幅%': round(float(stage.get('ret60')),2) if pd.notna(stage.get('ret60')) else '',
        '近20日位置%': round(float(stage.get('pos20'))*100,1) if pd.notna(stage.get('pos20')) else '',
        '60日高点回撤%': round(float(stage.get('drawdown60_pct')),2) if pd.notna(stage.get('drawdown60_pct')) else '',
        '稳定性分': round(stability_score, 1),
        '稳定性判断': defensive.get('state', '数据不足'),
        '弱市跑赢率%': round(float(defensive.get('outperform_ratio')), 1) if pd.notna(defensive.get('outperform_ratio')) else '',
        '弱市不跌率%': round(float(defensive.get('hold_ratio')), 1) if pd.notna(defensive.get('hold_ratio')) else '',
        '弱市相对优势%': round(float(defensive.get('bad_day_rel_edge')), 2) if pd.notna(defensive.get('bad_day_rel_edge')) else '',
        '相对20日稳定性%': round(float(defensive.get('rel20')), 2) if pd.notna(defensive.get('rel20')) else '',
        '相对60日稳定性%': round(float(defensive.get('rel60')), 2) if pd.notna(defensive.get('rel60')) else '',
        '20日波动率%': round(float(defensive.get('vol20')), 2) if pd.notna(defensive.get('vol20')) else '',
        '下行Beta': round(float(defensive.get('downside_beta')), 2) if pd.notna(defensive.get('downside_beta')) else '',
        '稳定60日最大回撤%': round(float(defensive.get('drawdown60')), 2) if pd.notna(defensive.get('drawdown60')) else '',
        '防守样本数': int(defensive.get('bad_days') or 0),
        '防守门槛结果': params.get('_defensive_gate_reason', '历史防守仅作背景分'),
        '策略类型': '弱市防守型' if defensive_track else 'MACD修复型',
        '策略环境': regime_now,
        '当前强弱分': round(current_score, 1),
        '当前强弱判断': current.get('state', '数据不足'),
        '当前相对大盘1日%': round(float(current.get('rel1')), 2) if pd.notna(current.get('rel1')) else '',
        '当前相对大盘5日%': round(float(current.get('rel5')), 2) if pd.notna(current.get('rel5')) else '',
        '当前相对大盘20日%': round(float(current.get('rel20')), 2) if pd.notna(current.get('rel20')) else '',
        '当前5日涨幅%': round(float(current.get('ret5')), 2) if pd.notna(current.get('ret5')) else '',
        '当前20日涨幅%': round(float(current.get('ret20')), 2) if pd.notna(current.get('ret20')) else '',
        '当前10日回撤%': round(float(current.get('dd10')), 2) if pd.notna(current.get('dd10')) else '',
        '当前10日最大下杀%': round(float(current.get('down_shock10')), 2) if pd.notna(current.get('down_shock10')) else '',
        '当前10日大跌次数': int(current.get('down3_count10') or 0),
        '当前强弱依据': current.get('reason', ''),
        ...
        '稳健优选分': round(float(np.clip(robust_score, 0, 100)), 1),
...
def screen_stock(code, name, qfq_df, raw_df, params, market_ctx=None, profile=None, sector_ctx=None):
    ...
    if not check_recent_gain_limit(raw_df, params['recent_days'], params['max_daily_gain']):
        return None
    # V1.48：策略分流。
    # 强市/震荡：延续原来的MACD修复主策略；
    # 弱市/极弱市：允许“防守型分流”，不要求一定处于MACD死叉修复，但必须通过当前韧性闸门。
    mr = macd_repair_state(qfq_df)
    ps = signal_day_price_state(raw_df)
    regime_now, _ = regime_pick_limit(market_ctx, params)
    defensive_track = bool(
        regime_now in ('弱市', '极弱市') and
        params.get('weak_allow_defensive_track', True)
    )
    if not defensive_track:
        if params.get('macd_stage_hard_gate', True):
            ok_stage, stage, _stage_reason = macd_stage_gate(qfq_df, params)
            if not ok_stage:
                return None
    ...
    # V1.48：历史防守只做背景分；真正的去留由“当前强弱 + 当前市场状态”决定。
    defensive = calculate_defensive_resilience(raw_df, qfq_df, market_ctx, params)
    current = calculate_current_resilience(raw_df, qfq_df, market_ctx, defensive, params)
    ok_current, current_reason = current_resilience_gate(current, market_ctx, params)
    if not ok_current:
        return None

    # V1.48：弱市防守型进一步确认“现在真的稳”，不是单纯MACD反抽。
    if defensive_track:
        cscore = safe_float(current.get('score')) or 0.0
        min_cs = float(params.get('defensive_track_min_score_very_weak' if regime_now == '极弱市' else 'defensive_track_min_score_weak'))
        min_r5 = float(params.get('defensive_track_min_rel5_very_weak' if regime_now == '极弱市' else 'defensive_track_min_rel5_weak'))
        min_r20 = float(params.get('defensive_track_min_rel20_very_weak' if regime_now == '极弱市' else 'defensive_track_min_rel20_weak'))
        max_today = float(params.get('defensive_track_max_today_gain_very_weak' if regime_now == '极弱市' else 'defensive_track_max_today_gain_weak'))
        max_ret20 = float(params.get('defensive_track_max_ret20_very_weak' if regime_now == '极弱市' else 'defensive_track_max_ret20_weak'))
        max_dd60 = float(params.get('defensive_track_max_dd60_very_weak' if regime_now == '极弱市' else 'defensive_track_max_dd60_weak'))
        max_down3 = int(params.get('defensive_track_max_down3_very_weak' if regime_now == '极弱市' else 'defensive_track_max_down3_weak'))
        if cscore < min_cs:
            return None
        if safe_float(current.get('rel5')) is None or safe_float(current.get('rel5')) < min_r5:
            return None
        if safe_float(current.get('rel20')) is None or safe_float(current.get('rel20')) < min_r20:
            return None
        if safe_float(current.get('today_pct')) is not None and current.get('today_pct') > max_today:
            return None
        if safe_float(current.get('ret20')) is not None and current.get('ret20') > max_ret20:
            return None
        if safe_float(current.get('dd60')) is not None and current.get('dd60') < max_dd60:
            return None
        if int(current.get('down3_count10') or 0) > max_down3:
            return None
    local_params = dict(params)
    local_params['_defensive_resilience'] = defensive
    local_params['_current_resilience'] = current
    local_params['_defensive_gate_reason'] = current_reason
    return build_result(code, name, raw_df, qfq_df, local_params, market_ctx, profile, sector_ctx)
...
def screen_stocks(data_manager, params, screen_date):
    ...
    market_ctx['daily_series'] = _market_daily_series(raw)
    profiles=enrich_profiles_before_screen(data_manager,screen_date,codes)
    sector_stats=sector_context_map(raw, qfq, profiles)
    results=[]; cap_filtered=0; cap_missing=0; base_candidates=0; defensive_rejected=0
    for code,qfq_df in qfq.items():
        ...
        try:
            ...
            # 单只股票数据异常不得中断整批筛选。记录代码/名称/异常，便于后续定位数据质量问题。
            logger.warning(f'筛选跳过异常股票 {code} {names.get(code, "")}：{type(exc).__name__}: {exc}')
            result=None
        if result: results.append(result)
        else: defensive_rejected += 1
    df=pd.DataFrame(results)
    base_candidate_count = len(df)
    regime, pick_limit = regime_pick_limit(market_ctx, params)
    if not df.empty:
        # V1.48：先按当前强弱排序，再按历史防守/策略质量补充排序；最终池按市场状态收缩。
        df=df.sort_values(['稳健优选分','当前强弱分','稳定性分','极度推荐分','推荐度','严格匹配度'],ascending=[False,False,False,False,False,False]).reset_index(drop=True)
        if pick_limit >= 0:
            df=df.head(pick_limit).copy()
        df=apply_extreme_picks(df, params.get('extreme_pick_count', 5))
        df.insert(0,'排名',range(1,len(df)+1))
    df.attrs['market_cap_filtered']=cap_filtered; df.attrs['market_cap_missing']=cap_missing; df.attrs['base_candidates']=base_candidate_count; df.attrs['defensive_rejected']=defensive_rejected; df.attrs['regime']=regime; df.attrs['regime_pick_limit']=pick_limit
    return df
...
class StockResearchGUI:
    ...
    def build_review_tab(self):
        ...
        table_frame = ttk.LabelFrame(body, text='稳健候选（按市场状态自动收缩；默认每页10只；另设极度推荐 TOP 5）', padding=5)
        ...
        columns = ('排名','极度推荐','稳健优选分','当前强弱分','当前强弱判断','稳定性分','代码','名称','极度推荐分','推荐度','当前相对大盘5日%','当前相对大盘20日%','当前10日回撤%','当前10日最大下杀%','当前10日大跌次数','弱市跑赢率%','弱市不跌率%','弱市相对优势%','稳定性判断','启动评分','涨停基因分','MACD阶段','MACD距','MACD距变化2日%','绿柱连续缩短','修复质量','前期形态','最新价','今日涨跌%','MA5状态','RSI14','KDJ状态','量比','5/20量比','20日涨幅%','60日高点回撤%','近一年涨停次数','相对强度','市场状态','收盘交易状态','关注理由')
        ...
        widths = {
            '排名':50,'极度推荐':76,'稳健优选分':88,'当前强弱分':82,'当前强弱判断':105,'稳定性分':78,'代码':72,'名称':90,'极度推荐分':86,'推荐度':70,'弱市跑赢率%':88,'弱市不跌率%':88,'弱市相对优势%':92,'稳定性判断':130,'启动评分':76,'涨停基因分':82,
            'MACD阶段':90,'MACD距':78,'MACD距变化2日%':96,'绿柱连续缩短':88,'修复质量':78,'前期形态':118,
            '最新价':72,'今日涨跌%':82,'MA5状态':110,'RSI14':68,'KDJ状态':82,'量比':72,'5/20量比':82,
            '20日涨幅%':84,'60日高点回撤%':100,'近一年涨停次数':94,'相对强度':78,'市场状态':110,'收盘交易状态':220,'关注理由':260
        }
        for c in columns:
    ...
    def read_params(self):
        try:
            return {
                ...
                'defensive_enabled': bool(DEFAULT_PARAMS.get('defensive_enabled', True)),
                'defensive_min_history_days': int(DEFAULT_PARAMS.get('defensive_min_history_days', 60)),
            }
        except Exception as exc:
    ...
    def refresh_market_profiles_thread(self):
        def worker():
            try:
                # V1.48：先走东方财富公开画像，不因为市值/行业刷新而强制安装AKShare。
    ...
    def _screen_worker(self, params, target):
        try:
            ...
            sector_stats=sector_context_map(raw, qfq, profiles)
            results=[]; cap_filtered=0; cap_missing=0; base_candidates=0; defensive_rejected=0; total=len(qfq)
            ...
            df=pd.DataFrame(results)
            if not df.empty:
                df=df.sort_values(['稳健优选分','稳定性分','极度推荐分','推荐度','严格匹配度'],ascending=[False,False,False,False,False]).reset_index(drop=True)
                df=apply_extreme_picks(df, params.get('extreme_pick_count', 5))
                df.insert(0,'排名',range(1,len(df)+1))
            ...
            self.set_status(f'{target} V1.48筛选完成：{len(df)}只｜当前强弱+稳健优选排序｜{extra}')
        except Exception as exc:
    ...
    def display_review_results(self):
        ...
        top5=self.result_df.sort_values(['稳健优选分','稳定性分','极度推荐分','推荐度'],ascending=[False,False,False,False],na_position='last').head(5)
        top_items=[]
        for i,(_,r) in enumerate(top5.iterrows(),1):
            top_items.append(f"{i}.{r['代码']} {r['名称']}({float(r.get('极度推荐分',0)):.1f}|{r.get('MACD阶段','')}|绿柱{r.get('绿柱连续缩短',0)}日)")
    ...
    def show_review_detail(self,row):
        ...
        for title,keylist in [
            ...
            ('核心指标',['最新价','今日涨跌%','今日状态','极度推荐','稳健优选分','稳定性分','稳定性判断','弱市跑赢率%','弱市不跌率%','弱市相对优势%','相对20日稳定性%','相对60日稳定性%','20日波动率%','下行Beta','稳定60日最大回撤%','防守样本数','防守门槛结果','极度推荐分','极度推荐理由','推荐度','涨停基因分','涨停基因状态','涨停基因说明','启动评分','基础策略分','严格匹配度','RSI14','KDJ状态','MA5状态','换手率','板块名称','板块状态','板块当日中位涨跌%','板块5日中位涨跌%','行业/主题','流通市值(亿)','参考总市值(亿)','市值状态','市场评分','市场状态','相对强度','相对大盘5日%','MACD零轴','DIF','DEA','MACD柱','近一年涨停次数','量比','5/20量比','20日涨幅%','近10日最大涨幅%']),
    ...
    def build_backtest_tab(self):
        ...
        pick_frame=ttk.LabelFrame(body,text='推荐结果：可选择按极度推荐或推荐度显示Top N',padding=5)
        daily_frame=ttk.LabelFrame(body,text='区间逐日统计：单日平均涨幅 + 各日累计平均涨幅（便于核对）',padding=5)
        result_frame=ttk.LabelFrame(body,text='未来6个交易日验证｜逐日涨幅 + 累计涨幅 + 交易模拟：只用于验证，不参与推荐',padding=5)
        body.add(pick_frame,weight=3); body.add(daily_frame,weight=2); body.add(result_frame,weight=4)
        pick_cols=('日期','排名','极度推荐排名','稳健优选分','稳定性分','弱市跑赢率%','弱市不跌率%','弱市相对优势%','极度推荐分','涨停基因分','代码','名称','推荐度','启动评分','基础策略分','严格匹配度','最新价','今日涨跌%','MA5状态','RSI14','KDJ状态','换手率','板块名称','板块状态','激进买点','保守买点','激进止损','保守止损','激进止盈','保守止盈','激进周期','保守周期','市场评分','市场状态','相对强度','行业/主题','流通市值(亿)','参考总市值(亿)','筹码结构','关注理由')
        ...
        daily_cols=('日期','候选数','原始候选数','冷却跳过','TopN','T+1单日平均%','T+2单日平均%','T+3单日平均%','T+4单日平均%','T+5单日平均%','T+1累计平均%','T+2累计平均%','T+3累计平均%','T+4累计平均%','T+5累计平均%','T+1上涨率','T+2上涨率','T+3上涨率','T+4上涨率','T+5上涨率','T+5涨停率','T+5有效样本数','T+6有效样本数','状态')
        self.backtest_daily_tree=ttk.Treeview(daily_frame,columns=daily_cols,show='headings')
        dw={'日期':90,'候选数':65,'原始候选数':85,'冷却跳过':75,'TopN':55,'T+1单日平均%':88,'T+2单日平均%':88,'T+3单日平均%':88,'T+4单日平均%':88,'T+5单日平均%':88,'T+1累计平均%':88,'T+2累计平均%':88,'T+3累计平均%':88,'T+4累计平均%':88,'T+5累计平均%':88,'T+1上涨率':78,'T+2上涨率':78,'T+3上涨率':78,'T+4上涨率':78,'T+5上涨率':78,'T+5涨停率':78,'T+5有效样本数':92,'T+6有效样本数':92,'状态':90}
        for c in daily_cols: self.backtest_daily_tree.heading(c,text=c); self.backtest_daily_tree.column(c,width=dw.get(c,85),anchor=tk.CENTER)
        dys=ttk.Scrollbar(daily_frame,orient=tk.VERTICAL,command=self.backtest_daily_tree.yview); dxs=ttk.Scrollbar(daily_frame,orient=tk.HORIZONTAL,command=self.backtest_daily_tree.xview)
        self.backtest_daily_tree.configure(yscrollcommand=dys.set,xscrollcommand=dxs.set); self.backtest_daily_tree.pack(fill=tk.BOTH,expand=True); dys.pack(side=tk.RIGHT,fill=tk.Y); dxs.pack(side=tk.BOTTOM,fill=tk.X)

        result_cols=('推荐日','推荐排名','推荐度','未来数据覆盖','截至最新日收益%','代码','名称','推荐日收盘','T+1单日涨幅%','T+2单日涨幅%','T+3单日涨幅%','T+4单日涨幅%','T+5单日涨幅%','T+6单日涨幅%','T+1累计涨幅%','T+2累计涨幅%','T+3累计涨幅%','T+4累计涨幅%','T+5累计涨幅%','T+6累计涨幅%','累计校验最大差异%','T+1开盘买入','买入价','交易结果','持有交易日','卖出价','净收益%','第一止盈%','第二止盈%','软止损%','硬止损%','首止盈卖出仓位%','首次止盈日','最终清仓日','最高浮盈%','最大浮亏%','交易备注','原T+5收盘%')
        self.backtest_result_tree=ttk.Treeview(result_frame,columns=result_cols,show='headings')
        rw={'推荐日':90,'推荐排名':75,'推荐度':65,'代码':75,'名称':95,'推荐日收盘':82,'累计校验最大差异%':92,'T+1开盘买入':105,'买入价':80,'交易结果':105,'持有交易日':80,'卖出价':80,'净收益%':80,'第一止盈%':80,'第二止盈%':80,'软止损%':75,'硬止损%':75,'首止盈卖出仓位%':95,'首次止盈日':90,'最终清仓日':90,'最高浮盈%':90,'最大浮亏%':90,'交易备注':220,'原T+5收盘%':90}
    ...
    def _simulate_trade(self, rdf, signal_day, target_pct, target2_pct,
        ...
        # V1.48：如果未来不足完整5日，不得伪装成“5日/6日收盘退出”。
        last = future.iloc[-1]; close = safe_float(last['收盘'])
        if close is None:
            base['交易结果']='无法退出'; return base
        horizon = len(future)
        if horizon < 5:
            base['交易结果'] = f'数据不足：仅有T+{horizon}后续交易日'
            base['持有交易日'] = horizon
            base['卖出价'] = ''
            base['净收益%'] = ''
            base['截至最新日收益%'] = round(((close/buy-1)*100),2) if buy else ''
            base['最终清仓日'] = ''
            base['最高浮盈%'] = round(best,2) if best > -998 else ''
            base['最大浮亏%'] = round(worst,2) if worst < 998 else ''
            base['交易备注'] = '；'.join(notes+[f'仅有{horizon}个后续交易日，未完成完整5日交易模拟'])
            return base
        if remain > 0:
            realized += remain*((close/buy-1)*100)
            remain = 0.0
        final_day=f'T+{horizon}'
        result=f'首止盈后T+{horizon}收盘退出' if first_done else f'T+{horizon}收盘退出'
        if not notes: notes.append('5日内未触发关键退出信号')
        return {
            **base,'交易结果':result,'持有交易日':horizon,
            '卖出价':round(close,2),'净收益%':round(realized-cost_pct,2),
            '截至最新日收益%':round(((close/buy-1)*100),2) if buy else '',
            '首次止盈日':first_day,'最终清仓日':final_day,
            '最高浮盈%':round(best,2),'最大浮亏%':round(worst,2),
            '交易备注':'；'.join(notes)
        }
    ...
    def _future_metrics(self,rdf,day,record):
        """回测未来6个交易日明细：同时给出逐日涨跌与从推荐日收盘计算的累计涨跌。

        口径：
        - T+N单日涨幅：T+N收盘 / T+(N-1)收盘 - 1；T+1的上一日为推荐日收盘。
        - T+N累计涨幅：T+N收盘 / 推荐日收盘 - 1。
        两者在未四舍五入的情况下应严格一致；同时输出累计校验差异，便于核查。
        """
        ts=pd.Timestamp(day); base=rdf[rdf['日期']==ts]
        empty={
            '推荐日收盘':'','T+1单日涨幅%':'','T+2单日涨幅%':'','T+3单日涨幅%':'','T+4单日涨幅%':'','T+5单日涨幅%':'','T+6单日涨幅%':'',
            'T+1累计涨幅%':'','T+2累计涨幅%':'','T+3累计涨幅%':'','T+4累计涨幅%':'','T+5累计涨幅%':'','T+6累计涨幅%':'',
            '累计校验最大差异%':'','未来数据覆盖':'',
        ...
        record['未来数据覆盖'] = '；'.join([f'T+{i+1}:有' for i in range(len(future))] + [f'T+{i+1}:无' for i in range(len(future),6)])
        daily_vals={}; cumulative_vals={}; compounded_vals={}
        prev_close=b
        if b and not future.empty:
            for n in range(1, len(future)+1):
                c=safe_float(future.iloc[n-1].get('收盘'))
                if c is None or prev_close is None or prev_close == 0:
                    prev_close = c
                    continue
                daily = (c / prev_close - 1) * 100.0
                cumulative = (c / b - 1) * 100.0
                daily_vals[n]=daily
                cumulative_vals[n]=cumulative
                if n == 1:
                    compounded_vals[n]=daily
                else:
                    prior = compounded_vals.get(n-1)
                    compounded_vals[n] = ((1 + prior/100.0) * (1 + daily/100.0) - 1) * 100.0 if prior is not None else cumulative
                prev_close = c
        for n in range(1,7):
            record[f'T+{n}单日涨幅%']=round(daily_vals[n],2) if n in daily_vals else ''
            record[f'T+{n}累计涨幅%']=round(cumulative_vals[n],2) if n in cumulative_vals else ''
        if cumulative_vals:
            diffs=[abs(cumulative_vals[n]-compounded_vals.get(n,cumulative_vals[n])) for n in cumulative_vals]
            record['累计校验最大差异%']=round(max(diffs),8) if diffs else 0.0
    ...
    def _backtest_worker(self,db_path,start,end,topn,mode,rank_mode,params,meta,trade_target,trade_target2,soft_stop,trade_stop,partial_pct,trade_cost,cooldown_days):
        try:
            ...
            last_signal_index={}
            market_daily_series_full = _market_daily_series(rmap) if params.get('market_environment_enabled',True) else pd.DataFrame()
            for di,day in enumerate(dates,1):
                ...
                if not market_daily_series_full.empty:
                    backtest_market_ctx['daily_series'] = market_daily_series_full[market_daily_series_full['日期']<=ts].copy()
                else:
                    backtest_market_ctx['daily_series'] = pd.DataFrame()
                sh_map = {}
                try:
                ...
                if cdf.empty:
                    daily_rows.append({'日期':day,'候选数':0,'原始候选数':0,'冷却跳过':0,'TopN':0,'T+1上涨率':'','T+2上涨率':'','T+3上涨率':'','T+4上涨率':'','T+5上涨率':'','T+1平均收益%':'','T+2平均收益%':'','T+3平均收益%':'','T+4平均收益%':'','T+5平均收益%':'','T+1单日平均%':'','T+2单日平均%':'','T+3单日平均%':'','T+4单日平均%':'','T+5单日平均%':'','T+1累计平均%':'','T+2累计平均%':'','T+3累计平均%':'','T+4累计平均%':'','T+5累计平均%':'','T+5涨停率':'','T+5有效样本数':0,'T+6有效样本数':0,'状态':'无候选'}); continue
                ...
                if cooldown_days>0:
                    ...
                    skipped=0
                if cdf.empty:
                    daily_rows.append({'日期':day,'候选数':0,'原始候选数':original_candidate_count,'冷却跳过':skipped,'TopN':0,'T+1上涨率':'','T+2上涨率':'','T+3上涨率':'','T+4上涨率':'','T+5上涨率':'','T+1平均收益%':'','T+2平均收益%':'','T+3平均收益%':'','T+4平均收益%':'','T+5平均收益%':'','T+1单日平均%':'','T+2单日平均%':'','T+3单日平均%':'','T+4单日平均%':'','T+5单日平均%':'','T+1累计平均%':'','T+2累计平均%':'','T+3累计平均%':'','T+4累计平均%':'','T+5累计平均%':'','T+5涨停率':'','T+5有效样本数':0,'T+6有效样本数':0,'状态':'全部被冷却'}); continue
                regime, regime_limit = regime_pick_limit(backtest_market_ctx, params)
                if rank_mode=='极度推荐':
                    cdf=cdf.sort_values(['稳健优选分','当前强弱分','稳定性分','极度推荐分','推荐度'],ascending=[False,False,False,False,False],na_position='last').reset_index(drop=True)
                else:
                    cdf=cdf.sort_values(['推荐度','当前强弱分','稳健优选分','启动评分','严格匹配度'],ascending=[False,False,False,False,False],na_position='last').reset_index(drop=True)
                cdf=cdf.head(regime_limit).copy()
                ...
                for _,rec in picks.iterrows():
                    code=normalize_code(rec['代码']); trade=self._simulate_trade(rmap.get(code,pd.DataFrame()),day,trade_target,trade_target2,soft_stop,trade_stop,partial_pct,trade_cost); pick_rows.append({'日期':day,'策略类型':rec.get('策略类型',''),'策略环境':rec.get('策略环境',''),'排名':rec['排名'],'极度推荐排名':rec.get('极度推荐排名',''),'稳健优选分':rec.get('稳健优选分',''),'稳定性分':rec.get('稳定性分',''),'弱市跑赢率%':rec.get('弱市跑赢率%',''),'弱市不跌率%':rec.get('弱市不跌率%',''),'弱市相对优势%':rec.get('弱市相对优势%',''),'极度推荐分':rec.get('极度推荐分',''),'涨停基因分':rec.get('涨停基因分',''),'代码':code,'名称':rec.get('名称',''),'推荐度':rec.get('推荐度',''),'严格匹配度':rec.get('严格匹配度',''),'最新价':rec.get('最新价',''),'今日涨跌%':rec.get('今日涨跌%',''),'MA5状态':rec.get('MA5状态',''),'RSI14':rec.get('RSI14',''),'KDJ状态':rec.get('KDJ状态',''),'换手率':rec.get('换手率',''),'板块名称':rec.get('板块名称',''),'板块状态':rec.get('板块状态',''),'筹码结构':rec.get('筹码结构',''),'行业/主题':rec.get('行业/主题',''),'流通市值(亿)':rec.get('流通市值(亿)',''),'参考总市值(亿)':rec.get('参考总市值(亿)',''),'收盘交易状态':rec.get('收盘交易状态',''),'激进买点':rec.get('激进买点',''),'保守买点':rec.get('保守买点',''),'激进止损':rec.get('激进止损',''),'保守止损':rec.get('保守止损',''),'激进止盈':rec.get('激进止盈',''),'保守止盈':rec.get('保守止盈',''),'关注理由':rec.get('关注理由','')}); base=self._future_metrics(rmap.get(code,pd.DataFrame()),day,{'推荐日':day,'推荐排名':rec['排名'],'极度推荐排名':rec.get('极度推荐排名',''),'极度推荐分':rec.get('极度推荐分',''),'涨停基因分':rec.get('涨停基因分',''),'推荐度':rec.get('推荐度',''),'代码':code,'名称':rec.get('名称','')});
                ...
                if mode=='日期区间批量验证':
                    ...
                    daily_rows.append({'日期':day,'候选数':len(cdf),'原始候选数':original_candidate_count,'冷却跳过':skipped,'TopN':len(picks),'T+1上涨率':rate(vals[1]),'T+2上涨率':rate(vals[2]),'T+3上涨率':rate(vals[3]),'T+4上涨率':rate(vals[4]),'T+5上涨率':rate(vals[5]),'T+1平均收益%':avg(vals[1]),'T+2平均收益%':avg(vals[2]),'T+3平均收益%':avg(vals[3]),'T+4平均收益%':avg(vals[4]),'T+5平均收益%':avg(vals[5]),'T+1单日平均%':avg([safe_float(x.get('T+1单日涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+1单日涨幅%')) is not None]),'T+2单日平均%':avg([safe_float(x.get('T+2单日涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+2单日涨幅%')) is not None]),'T+3单日平均%':avg([safe_float(x.get('T+3单日涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+3单日涨幅%')) is not None]),'T+4单日平均%':avg([safe_float(x.get('T+4单日涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+4单日涨幅%')) is not None]),'T+5单日平均%':avg([safe_float(x.get('T+5单日涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+5单日涨幅%')) is not None]),'T+1累计平均%':avg([safe_float(x.get('T+1累计涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+1累计涨幅%')) is not None]),'T+2累计平均%':avg([safe_float(x.get('T+2累计涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+2累计涨幅%')) is not None]),'T+3累计平均%':avg([safe_float(x.get('T+3累计涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+3累计涨幅%')) is not None]),'T+4累计平均%':avg([safe_float(x.get('T+4累计涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+4累计涨幅%')) is not None]),'T+5累计平均%':avg([safe_float(x.get('T+5累计涨幅%')) for _,x in latest.iterrows() if safe_float(x.get('T+5累计涨幅%')) is not None]),'T+5涨停率':round(sum(v>=9.8 for v in highs)/len(highs)*100,1) if highs else '','T+5有效样本数':len(vals[5]),'T+6有效样本数':len([1 for _,rr in latest.iterrows() if safe_float(rr.get('T+6累计涨幅%')) is not None]),'状态':f'有效｜{regime}'})
                if mode=='日期区间批量验证':
            ...
            if not trade_df.empty and '净收益%' in trade_df.columns:
                net_series=pd.to_numeric(trade_df['净收益%'],errors='coerce').dropna()
                wins=int((net_series>0).sum()); total_tr=len(net_series); avg_net=float(net_series.mean()) if total_tr else 0.0
                incomplete=int(trade_df['交易结果'].astype(str).str.startswith('数据不足').sum()) if '交易结果' in trade_df.columns else 0
                trade_summary=f'交易模拟：有效{total_tr}笔｜数据不足{incomplete}笔｜TP1 {trade_target:.1f}%/{partial_pct:.0f}%仓｜TP2 {trade_target2:.1f}%｜软止损{soft_stop:.1f}%｜硬止损{trade_stop:.1f}%｜成本{trade_cost:.2f}%｜冷却{cooldown_days}天｜有效盈利率{wins/max(1,total_tr)*100:.1f}%｜有效平均净收益{avg_net:.2f}%'
            else: trade_summary='交易模拟：暂无完整有效交易样本'
            if mode=='单日验证': self.ui(self.backtest_summary_var.set,f'{start}：候选{len(candidates)}只｜{rank_mode} Top{topn}｜数据库股票池{meta["stock_count"]}只｜{trade_summary}'); self.set_status(f'历史验证完成：{start}；排序口径：{rank_mode}。')
                ...
                rd=pd.DataFrame(result_rows)
                cov1=int(pd.to_numeric(rd.get('T+1累计涨幅%',pd.Series(dtype=float)),errors='coerce').notna().sum()) if not rd.empty else 0
                cov5=int(pd.to_numeric(rd.get('T+5累计涨幅%',pd.Series(dtype=float)),errors='coerce').notna().sum()) if not rd.empty else 0
                self.ui(self.backtest_summary_var.set,f'区间完成：{start}~{end}｜交易日{len(dates)}｜推荐明细{len(result_rows)}条｜T+1有效{cov1}｜T+5有效{cov5}｜{trade_summary}')
                self.set_status(f'历史区间验证完成：{start}~{end}。')
        except Exception as exc:
    ...
    def _fill_backtest_trees(self):
        ...
        for _,row in self.backtest_daily_df.iterrows(): self.backtest_daily_tree.insert('',tk.END,values=[row.get(c,'') for c in self.backtest_daily_tree['columns']])
    ...
    def _prepare_v145_export_dataframe(self, df):
        """V1.48：只导出真实可计算、与人工复盘直接相关的字段。"""
        columns = [
            '排名','代码','名称','极度推荐','极度推荐排名','稳健优选分','稳定性分','稳定性判断','弱市跑赢率%','弱市不跌率%','弱市相对优势%','相对20日稳定性%','相对60日稳定性%','20日波动率%','下行Beta','稳定60日最大回撤%','防守样本数','防守门槛结果','极度推荐分','启动评分','推荐度','涨停基因分',
            'MACD阶段','MACD距','MACD距变化1日%','MACD距变化2日%','绿柱连续缩短','修复质量','前期形态',
            '60日涨幅%','60日高点回撤%','近20日位置%','最新价','今日涨跌%','今日状态','MA5状态','MA5','MA10','MA20','MA60',
            'RSI14','RSI状态','K值','D值','J值','KDJ状态','量比','5/20量比','20日涨幅%','近10日最大涨幅%',
            '成交额(万)','近一年涨停次数','相对强度','相对大盘1日%','相对大盘5日%','相对大盘20日%',
            '市场评分','市场状态','严格匹配度','收盘交易状态'
        ]
        out = df[[c for c in columns if c in df.columns]].copy()
        # 行业字段只有真实数据覆盖率达到30%时才导出；否则直接省略。
        if '所属行业' in df.columns:
            s = df['所属行业'].astype(str).str.strip()
            valid = (~s.isin(['','暂无','暂无同日行业数据','未分类','nan','None'])).sum()
            if valid / max(1,len(df)) >= 0.30:
                insert_at = min(18, len(out.columns))
                out.insert(insert_at, '所属行业', df['所属行业'].values)
        return out
    ...
    def export_csv(self):
        ...
        if path:
            export_df = self._prepare_v145_export_dataframe(self.result_df)
