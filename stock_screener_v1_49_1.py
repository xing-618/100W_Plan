...
# ============================================================
# A股短线研究工具 V1.49.1：V1.45筛选内核 + 动态精筛/极度推荐重排
#
# V1.2 目标：
...
DEFAULT_PARAMS = {
    ...
    # V1.49：保留 V1.45 全部硬筛选条件；这里只控制最终精筛数量。
    'selection_max_total': 20,
    'selection_max_weak': 5,
    'selection_max_very_weak': 5,
    'selection_extreme_max_strong': 8,
    'selection_extreme_max_normal': 8,
    'selection_extreme_max_weak': 5,
    'selection_extreme_max_very_weak': 5,
    'selection_floor_strong': 66.0,
    'selection_floor_normal': 68.0,
    'selection_floor_weak': 70.0,
    'selection_floor_very_weak': 72.0,
    'selection_gap_strong': 8.0,
    'selection_gap_normal': 8.0,
    'selection_gap_weak': 6.0,
    'selection_gap_very_weak': 5.0,
    # 兼容旧配置读取；V1.49 不再强制固定 TopN。
    'extreme_pick_count': 8,
    ...
    'extreme_limit_gene_weight': 6.0,
    'extreme_context_weight': 4.0,
    'macd_tangle_ratio': 0.35,
    # V1.45：只把真正的“绿柱收缩+死叉收敛”视作主修复窗口。
...
def macd_stage_state(qfq_df, tangle_ratio=0.35):
    """V1.45：显式识别早期死叉修复、缠绕横盘、弱修复和前期大涨回撤。"""
    out = {
...
def turnover_rate_from_profile(raw_df, profile):
    """V1.45：用成交额/流通市值计算近似换手率，避免依赖额外接口。"""
    try:
...
def sector_context_map(raw_dict, qfq_dict, profiles):
    """V1.45：基于同日全市场股票画像+日线，给候选补充所属板块环境。"""
    groups = {}
...
def limit_up_gene_state(raw_df, threshold=9.8, lookback=LOOKBACK_TRADING_DAYS):
    """V1.45：把涨停基因从“涨停次数”升级为历史弹性、近期密度、距离最近涨停
    以及历史涨停后3个交易日的延续能力。
...
def calculate_extreme_score(raw_df, qfq_df, params, market_ctx=None, profile=None, sector_ctx=None):
    """V1.45：极度推荐只用实际日线可算的核心因子，不使用缺失市值/换手/行业默认分。"""
    ...
    # 近期位置比120日位置更贴近“低点企稳后准备启动”的人工看盘。
    ...
    weights={'macd':32.0,'ma5':10.0,'kdj':8.0,'rsi':8.0,'volume':12.0,'position':10.0,'gene':15.0,'context':5.0}
    ...
    return {'score':round(float(np.clip(score,0,100)),1),'reason':'；'.join(reasons[:8]),'gene':gene,'macd_stage':stage,'turnover':np.nan,'parts':{k:round(v,1) for k,v in parts.items()}}


def _v149_num(v, default=None):
    x = safe_float(v)
    return default if x is None else x


def _v149_piece(v, low, high, best_low=None, best_high=None):
    """把一个指标转换成 0~100 质量分；缺失值只给中性分，不抬高也不一票否决。"""
    x = safe_float(v)
    if x is None or not np.isfinite(x):
        return 50.0
    if best_low is not None and best_high is not None and best_low <= x <= best_high:
        return 100.0
    if x <= low or x >= high:
        return 25.0
    left = best_low if best_low is not None else (low + high) / 2.0
    right = best_high if best_high is not None else left
    if x < left:
        return 25.0 + 75.0 * (x - low) / max(1e-9, left - low)
    return 100.0 - 75.0 * (x - right) / max(1e-9, high - right)


def _v149_timing_score(today_pct):
    x = safe_float(today_pct)
    if x is None:
        return 50.0
    if 0.0 <= x <= 2.5:
        return 100.0
...
def _v149_structure_score(row):
    price = safe_float(row.get('最新价'))
    ma5 = safe_float(row.get('MA5')); ma10 = safe_float(row.get('MA10'))
    ma20 = safe_float(row.get('MA20')); ma60 = safe_float(row.get('MA60'))
    if price is None:
        return 50.0, 0
    score = 50.0
    confirms = 0
    if ma5 is not None:
        if price >= ma5: score += 15; confirms += 1
        elif price < ma5 * 0.985: score -= 12
    if ma10 is not None and ma20 is not None:
        if ma10 >= ma20: score += 15; confirms += 1
        elif ma10 < ma20 * 0.985: score -= 10
    if ma20 is not None:
        if price >= ma20: score += 15; confirms += 1
        elif price < ma20 * 0.985: score -= 13
    if ma20 is not None and ma60 is not None:
        if ma20 >= ma60: score += 15; confirms += 1
        elif ma20 < ma60 * 0.985: score -= 12
    return float(np.clip(score, 0, 100)), confirms


def _v149_stability_score(row):
    dd = safe_float(row.get('60日高点回撤%'))
    ret20 = safe_float(row.get('20日涨幅%'))
    pos20 = safe_float(row.get('近20日位置%'))
    if dd is None: dd_score = 55.0
    elif dd >= -8: dd_score = 100.0
    elif dd >= -15: dd_score = 88.0
    elif dd >= -22: dd_score = 72.0
    elif dd >= -28: dd_score = 55.0
    else: dd_score = 35.0
    if ret20 is None: ret_score = 55.0
    elif -5 <= ret20 <= 10: ret_score = 100.0
    elif 10 < ret20 <= 16: ret_score = 86.0
    elif 16 < ret20 <= 22: ret_score = 68.0
    elif 22 < ret20 <= 30: ret_score = 48.0
    elif ret20 > 30: ret_score = 30.0
    elif ret20 >= -10: ret_score = 72.0
    else: ret_score = 48.0
    if pos20 is None: pos_score = 55.0
    elif 25 <= pos20 <= 80: pos_score = 92.0
    elif 15 <= pos20 <= 88: pos_score = 76.0
    elif pos20 > 92: pos_score = 38.0
    else: pos_score = 52.0
    return float(np.clip(0.45*dd_score + 0.35*ret_score + 0.20*pos_score, 0, 100))


def _v149_current_strength(row):
    rs = safe_float(row.get('相对强度'))
    rel5 = safe_float(row.get('相对大盘5日%'))
    rel20 = safe_float(row.get('相对大盘20日%'))
    today = safe_float(row.get('今日涨跌%'))
    rs_part = 50.0 if rs is None else float(np.clip(rs, 0, 100))
    rel5_part = 50.0 if rel5 is None else float(np.clip(50 + rel5 * 3.0, 0, 100))
    rel20_part = 50.0 if rel20 is None else float(np.clip(50 + rel20 * 1.5, 0, 100))
    rel_score = 0.55*rs_part + 0.30*rel5_part + 0.15*rel20_part
    timing = _v149_timing_score(today)
    return float(np.clip(0.72*rel_score + 0.28*timing, 0, 100))


def _v149_oscillator_score(row):
    rsi = safe_float(row.get('RSI14'))
    k = safe_float(row.get('K值')); d = safe_float(row.get('D值')); j = safe_float(row.get('J值'))
    if rsi is None: rsi_score = 50.0
    elif 45 <= rsi <= 68: rsi_score = 100.0
    elif 35 <= rsi < 45 or 68 < rsi <= 75: rsi_score = 78.0
    elif rsi > 80: rsi_score = 28.0
    else: rsi_score = 50.0
    kd = 50.0
    if k is not None and d is not None:
        kd = 82.0 if k >= d else 38.0
    if j is not None:
        if j > 105: kd -= 20.0
        elif j < 20 and k is not None and d is not None and k > d: kd += 8.0
    return float(np.clip(0.60*rsi_score + 0.40*kd, 0, 100))


def _v149_volume_score(row):
    vol = safe_float(row.get('量比')); con = safe_float(row.get('5/20量比'))
    if vol is None: s = 50.0
    elif 0.85 <= vol <= 1.6: s = 95.0
    elif 0.7 <= vol < 0.85 or 1.6 < vol <= 2.0: s = 78.0
    elif vol < 0.55 or vol > 2.3: s = 35.0
    else: s = 55.0
    if con is not None:
        if 0.55 <= con <= 0.9: s += 5.0
        elif con > 1.25: s -= 8.0
        elif con < 0.35: s -= 5.0
    return float(np.clip(s, 0, 100))


def _v149_row_scores(row):
    current = _v149_current_strength(row)
    structure, struct_conf = _v149_structure_score(row)
    stability = _v149_stability_score(row)
    repair = _v149_num(row.get('修复质量'), 50.0)
    startup = _v149_num(row.get('启动评分'), 50.0)
    strict = _v149_num(row.get('严格匹配度'), 50.0)
    osc = _v149_oscillator_score(row)
    volume = _v149_volume_score(row)
    timing = _v149_timing_score(row.get('今日涨跌%'))
    gene = _v149_num(row.get('涨停基因分'), 50.0)
    # 极度推荐：从“会涨过几次”转为“现在是否同时满足修复、强弱、结构、稳定四个条件”。
    extreme = (
        0.23*repair + 0.22*current + 0.15*structure + 0.15*stability +
        0.08*startup + 0.05*strict + 0.05*osc + 0.03*timing +
        0.02*volume + 0.02*gene
    )
    base = _v149_num(row.get('基础策略分'), _v149_num(row.get('推荐度'), 50.0))
    quality = (
        0.34*extreme + 0.20*current + 0.11*startup + 0.10*base +
        0.08*strict + 0.10*stability + 0.07*structure
    )
    confirms = struct_conf
    if safe_float(row.get('相对大盘5日%')) is not None and safe_float(row.get('相对大盘5日%')) >= 0: confirms += 1
    if safe_float(row.get('相对大盘20日%')) is not None and safe_float(row.get('相对大盘20日%')) >= 0: confirms += 1
    if repair >= 85: confirms += 1
    if 0 <= _v149_num(row.get('今日涨跌%'), 0) <= 2.5: confirms += 1
    risk_flags=[]
    dd=safe_float(row.get('60日高点回撤%')); ret20=safe_float(row.get('20日涨幅%')); today=safe_float(row.get('今日涨跌%'))
    if dd is not None and dd < -28: risk_flags.append('60日回撤深')
    if ret20 is not None and ret20 > 25: risk_flags.append('20日涨幅过快')
    if today is not None and today > 3.5: risk_flags.append('推荐日偏追涨')
    if structure < 40: risk_flags.append('均线结构弱')
    if current < 40: risk_flags.append('当前相对偏弱')
    return {
        '极度推荐分': round(float(np.clip(extreme,0,100)),1),
        '优选分': round(float(np.clip(quality,0,100)),1),
        '当前强弱分': round(float(np.clip(current,0,100)),1),
        '结构确认分': round(float(np.clip(structure,0,100)),1),
        '稳定性分': round(float(np.clip(stability,0,100)),1),
        '当前温度分': round(float(np.clip(timing,0,100)),1),
        '确认项数': int(confirms),
        '当前风险提示': '；'.join(risk_flags[:4]),
    }


def _v149_market_profile(market_ctx, params):
    mc = market_ctx or {}
    score = _v149_num(mc.get('score'), 50.0)
    adv = _v149_num(mc.get('advance_ratio'))
    med = _v149_num(mc.get('median_pct'))
    if score >= 70 and ((adv is not None and adv >= 0.55) or (med is not None and med >= 0.25)):
        return '强市', 20, int(params.get('selection_extreme_max_strong',8)), float(params.get('selection_floor_strong',66)), float(params.get('selection_gap_strong',8))
    if score >= 50 and ((adv is None or adv >= 0.42) or (med is not None and med >= 0)):
        return '震荡市', 20, int(params.get('selection_extreme_max_normal',8)), float(params.get('selection_floor_normal',68)), float(params.get('selection_gap_normal',8))
    if score >= 35:
        return '弱市', 5, int(params.get('selection_extreme_max_weak',5)), float(params.get('selection_floor_weak',70)), float(params.get('selection_gap_weak',6))
    return '极弱市', 5, int(params.get('selection_extreme_max_very_weak',5)), float(params.get('selection_floor_very_weak',72)), float(params.get('selection_gap_very_weak',5))


def dynamic_select_candidates(df, market_ctx=None, params=None):
    """V1.49：V1.45硬筛选完全保留，只在最后一步做动态精筛。"""
    if df is None or df.empty:
        return df
    params = params or DEFAULT_PARAMS
    out = df.copy()
    profile_name, max_total, max_extreme, floor, gap = _v149_market_profile(market_ctx, params)
    scores=[]
    for _, row in out.iterrows():
        s=_v149_row_scores(row)
        # 保存 V1.45 原分，方便复盘“为什么原极度推荐没有抬高好股”。
        s['原极度推荐分']=row.get('极度推荐分','')
        scores.append(s)
    score_df=pd.DataFrame(scores,index=out.index)
    for c in score_df.columns:
        out[c]=score_df[c]
    out['极度推荐']=''; out['极度推荐排名']=''
    quality_num=pd.to_numeric(out['优选分'],errors='coerce')
    top=float(quality_num.max()) if quality_num.notna().any() else 0.0
    percentile_floor=float(quality_num.quantile(0.60)) if quality_num.notna().any() else floor
    dynamic_floor=max(floor, percentile_floor)
    qualified=out[(quality_num>=dynamic_floor) & (quality_num>=top-gap)].copy()
    # 不凑数：若顶部也很弱，只允许高于绝对门槛4分的少量救援候选。
    if qualified.empty and quality_num.notna().any():
        rescue=out[quality_num>=floor+4].copy()
        if not rescue.empty:
            qualified=rescue.sort_values(['优选分','极度推荐分'],ascending=False).head(min(3,max_total)).copy()
    if qualified.empty:
        out.attrs['v149_profile']=profile_name
        return out.iloc[0:0].copy()
    qualified=qualified.sort_values(['优选分','当前强弱分','极度推荐分','启动评分','严格匹配度'],ascending=False,na_position='last').head(max_total).reset_index(drop=True)
    # 极度推荐是“质量交集”，不是 TopN 截断。
    ext=pd.to_numeric(qualified['极度推荐分'],errors='coerce')
    ext_floor=max(76.0, float(ext.quantile(0.60)) if ext.notna().any() else 76.0)
    topq=float(pd.to_numeric(qualified['优选分'],errors='coerce').max())
    ext_gap=5.0 if profile_name in ('强市','震荡市') else 4.0
    ex=qualified[
        (ext>=ext_floor) &
        (pd.to_numeric(qualified['优选分'],errors='coerce')>=topq-ext_gap) &
        (pd.to_numeric(qualified['稳定性分'],errors='coerce')>=55) &
        (pd.to_numeric(qualified['结构确认分'],errors='coerce')>=45) &
        (pd.to_numeric(qualified['修复质量'],errors='coerce')>=78) &
        (pd.to_numeric(qualified['确认项数'],errors='coerce')>=3)
    ].sort_values(['极度推荐分','优选分','当前强弱分','稳定性分'],ascending=False,na_position='last').head(max_extreme)
    for rank, idx in enumerate(ex.index, 1):
        qualified.at[idx,'极度推荐']='极度推荐'
        qualified.at[idx,'极度推荐排名']=rank
    qualified.insert(0,'策略环境',profile_name)
    qualified.insert(1,'推荐数量上限',max_total)
    qualified.insert(2,'极度推荐上限',max_extreme)
    qualified.attrs['v149_profile']=profile_name
    return qualified


def safe_screen_stock(code, name, qfq_df, raw_df, params, market_ctx=None, profile=None, sector_ctx=None, context='筛选'):
    """单只股票异常只跳过该股票；缺失字段不会让整批筛选失败。"""
    try:
        return screen_stock(code, name, qfq_df, raw_df, params, market_ctx, profile, sector_ctx)
    except Exception as exc:
        logger.warning(f'[{context}] 单只股票跳过 {code} {name}: {exc}', exc_info=True)
        return None


def apply_extreme_picks(df, top_n=5):
    """V1.45：不减少候选池，只标记极度推荐 Top N。"""
...
def screen_stocks(data_manager, params, screen_date):
    ...
    if not df.empty:
        df=dynamic_select_candidates(df, market_ctx, params)
        if not df.empty:
...
class StockResearchGUI:
    ...
    def build_review_tab(self):
        ...
        table_frame = ttk.LabelFrame(body, text='动态精筛（总数≤20；强/震荡市极度推荐最多8只，弱市最多5只）', padding=5)
        ...
        columns = ('排名','极度推荐','极度推荐分','代码','名称','推荐度','启动评分','涨停基因分','MACD阶段','MACD距','MACD距变化2日%','绿柱连续缩短','修复质量','前期形态','最新价','今日涨跌%','MA5状态','RSI14','KDJ状态','量比','5/20量比','20日涨幅%','60日高点回撤%','近一年涨停次数','相对强度','市场状态','收盘交易状态')
        ...
        widths = {
            '排名':50,'极度推荐':76,'极度推荐分':86,'代码':72,'名称':90,'推荐度':70,'启动评分':76,'涨停基因分':82,
    ...
    def refresh_market_profiles_thread(self):
        def worker():
            try:
                # V1.45：先走东方财富公开画像，不因为市值/行业刷新而强制安装AKShare。
    ...
    def _screen_worker(self, params, target):
        try:
            ...
            for idx,code in enumerate(qfq.keys(),1):
                ...
                if params.get('market_cap_filter_enabled',False):
                    ...
                    if not same_day or cap is None:
                        ...
                        lo=float(params.get('min_market_cap',0) or 0); hi=float(params.get('max_market_cap',999999) or 999999)
                        if lo<=cap<=hi:
                            result=safe_screen_stock(code,names.get(code,''),qfq.get(code),raw.get(code),local_params,market_ctx,profile,get_stock_sector_context(code, profiles, sector_stats),context='收盘复盘')
                        else:
                            cap_filtered+=1
                else:
                    result=safe_screen_stock(code,names.get(code,''),qfq.get(code),raw.get(code),local_params,market_ctx,profile,get_stock_sector_context(code, profiles, sector_stats),context='收盘复盘')
            ...
            self.set_status(f'{target} 筛选完成：{len(df)}只｜V1.49动态精筛{extra}')
        except Exception as exc:
    ...
    def display_review_results(self):
        ...
        top5=self.result_df[self.result_df['极度推荐'].astype(str).str.strip().ne('')].sort_values(['极度推荐排名','极度推荐分','优选分'],ascending=[True,False,False],na_position='last') if '极度推荐' in self.result_df.columns else pd.DataFrame()
        top_items=[]
        for i,(_,r) in enumerate(top5.iterrows(),1):
            top_items.append(f"{i}.{r['代码']} {r['名称']}({float(r.get('极度推荐分',0) or 0):.1f}|优选{float(r.get('优选分',0) or 0):.1f})")
        mode_text='仅显示极度推荐' if self.review_extreme_only else f'当前{total}只'
        self.review_top5_label.set(f"极度推荐({len(top5)}只)：{'、'.join(top_items) if top_items else '无'}｜{mode_text}")
        ...
        try:
            ...
            for di,day in enumerate(dates,1):
                ...
                for idx,code in enumerate(all_codes,1):
                    ...
                    result=safe_screen_stock(code,names.get(code,''),q_until,r_until,dict(params),backtest_market_ctx,context=f'历史验证 {day}')
                ...
                cdf=dynamic_select_candidates(cdf, backtest_market_ctx, params)
                if cdf.empty:
                    daily_rows.append({'日期':day,'候选数':0,'原始候选数':original_candidate_count,'冷却跳过':skipped,'TopN':0,'T+1上涨率':'','T+2上涨率':'','T+3上涨率':'','T+4上涨率':'','T+5上涨率':'','T+1平均收益%':'','T+2平均收益%':'','T+3平均收益%':'','T+4平均收益%':'','T+5平均收益%':'','T+5涨停率':'','状态':'精筛后无高质量候选'}); continue
                if rank_mode=='极度推荐':
                    cdf=cdf[cdf['极度推荐'].astype(str).str.strip().ne('')].sort_values(['极度推荐排名','极度推荐分','优选分'],ascending=[True,False,False],na_position='last').reset_index(drop=True)
                else:
                    cdf=cdf.sort_values(['优选分','当前强弱分','启动评分','严格匹配度'],ascending=[False,False,False,False],na_position='last').reset_index(drop=True)
                if cdf.empty: continue
                cdf.insert(0,'排名',range(1,len(cdf)+1)); picks=cdf.head(min(topn,len(cdf)))
    ...
    def _prepare_v145_export_dataframe(self, df):
        """V1.45：只导出真实可计算、与人工复盘直接相关的字段。"""
        columns = [
            '排名','策略环境','推荐数量上限','极度推荐上限','代码','名称','极度推荐','极度推荐排名','优选分','极度推荐分','原极度推荐分','启动评分','推荐度','当前强弱分','结构确认分','稳定性分','确认项数','当前风险提示','涨停基因分',
