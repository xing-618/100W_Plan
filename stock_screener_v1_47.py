...
# ============================================================
# A股短线研究工具 V1.47：MACD修复 + 防守韧性筛选 + 稳健优选排序
#
# V1.2 目标：
...
DEFAULT_PARAMS = {
    ...
    # V1.47：防守韧性层——弱市不再只做加分，而是进入硬筛选；主列表优先按稳健优选分排序。
    'extreme_pick_count': 5,
    # V1.47 极度推荐：对应个人看盘框架，而不是单一指标排名。
    ...
    # V1.47：防守韧性层。目标是筛出“市场转弱时相对不容易跟跌”的历史行为。
    # 注意：这是历史行为代理，不保证未来不跌。
    'defensive_enabled': True,
    'defensive_min_history_days': 60,
    'defensive_normal_min_score': 58.0,
    'defensive_weak_min_score': 64.0,
    'defensive_very_weak_min_score': 70.0,
    'defensive_bad_day_threshold': -0.8,
    'defensive_hold_threshold': -0.5,
    'defensive_min_bad_days': 8,
    'defensive_min_outperform_ratio': 52.0,
    'defensive_min_rel20': 0.0,
    'defensive_min_rel60': 0.0,
    'defensive_max_drawdown60': -22.0,
    'defensive_max_downside_beta': 1.15,
    'defensive_weight': 50.0,
    'defensive_extreme_weight': 32.0,
    'defensive_strict_weight': 18.0,
}
...
def macd_stage_state(qfq_df, tangle_ratio=0.35):
    """V1.47：显式识别早期死叉修复、缠绕横盘、弱修复和前期大涨回撤。"""
    out = {
...
def turnover_rate_from_profile(raw_df, profile):
    """V1.47：用成交额/流通市值计算近似换手率，避免依赖额外接口。"""
    try:
...
def sector_context_map(raw_dict, qfq_dict, profiles):
    """V1.47：基于同日全市场股票画像+日线，给候选补充所属板块环境。"""
    groups = {}
...
def limit_up_gene_state(raw_df, threshold=9.8, lookback=LOOKBACK_TRADING_DAYS):
    """V1.47：把涨停基因从“涨停次数”升级为历史弹性、近期密度、距离最近涨停
    以及历史涨停后3个交易日的延续能力。
...
def _market_daily_series(raw_dict):
    """V1.47：构造目标日及以前的全市场日度中位涨跌、上涨占比等序列。"""
    rows = []
...
def calculate_extreme_score(raw_df, qfq_df, params, market_ctx=None, profile=None, sector_ctx=None, defensive=None):
    """V1.47：极度推荐在V1.47多因子基础上加入防守韧性，但不允许缺失数据凭空加分。"""
...
def build_result(code, name, raw_df, qfq_df, params, market_ctx=None, profile=None, sector_ctx=None):
    ...
    stable_weight = float(params.get('defensive_weight', 50.0))
    stable_extreme_weight = float(params.get('defensive_extreme_weight', 32.0))
    stable_strict_weight = float(params.get('defensive_strict_weight', 18.0))
    total_w = max(1.0, stable_weight + stable_extreme_weight + stable_strict_weight)
    robust_score = (stability_score * stable_weight + extreme_score * stable_extreme_weight + strict_norm * stable_strict_weight) / total_w
    # V1.47：极度推荐独立计算，不替换原推荐度。
    ...
    return {
        ...
        '防守门槛结果': params.get('_defensive_gate_reason', '防守韧性通过'),
...
def screen_stock(code, name, qfq_df, raw_df, params, market_ctx=None, profile=None, sector_ctx=None):
    ...
    # V1.47：传统信号通过后，再做防守韧性层。弱市时门槛自动提高。
    defensive = calculate_defensive_resilience(raw_df, qfq_df, market_ctx, params)
    ok_def, def_reason = defensive_hard_gate(defensive, market_ctx, params)
    if not ok_def:
        return None
    local_params = dict(params)
    local_params['_defensive_resilience'] = defensive
    local_params['_defensive_gate_reason'] = def_reason
...
def screen_stocks(data_manager, params, screen_date):
    ...
    if not df.empty:
        # V1.47：主列表改为稳健优选分优先；传统推荐度保留作辅助参考。
    ...
    df.attrs['market_cap_filtered']=cap_filtered; df.attrs['market_cap_missing']=cap_missing; df.attrs['base_candidates']=base_candidates; df.attrs['defensive_rejected']=defensive_rejected
    return df
...
class StockResearchGUI:
    ...
    def build_review_tab(self):
        ...
        columns = ('排名','极度推荐','稳健优选分','稳定性分','代码','名称','极度推荐分','推荐度','弱市跑赢率%','弱市不跌率%','弱市相对优势%','稳定性判断','启动评分','涨停基因分','MACD阶段','MACD距','MACD距变化2日%','绿柱连续缩短','修复质量','前期形态','最新价','今日涨跌%','MA5状态','RSI14','KDJ状态','量比','5/20量比','20日涨幅%','60日高点回撤%','近一年涨停次数','相对强度','市场状态','收盘交易状态','关注理由')
        ...
        widths = {
            '排名':50,'极度推荐':76,'稳健优选分':88,'稳定性分':78,'代码':72,'名称':90,'极度推荐分':86,'推荐度':70,'弱市跑赢率%':88,'弱市不跌率%':88,'弱市相对优势%':92,'稳定性判断':130,'启动评分':76,'涨停基因分':82,
    ...
    def refresh_market_profiles_thread(self):
        def worker():
            try:
                # V1.47：先走东方财富公开画像，不因为市值/行业刷新而强制安装AKShare。
    ...
    def _screen_worker(self, params, target):
        try:
            ...
            self.set_status(f'{target} V1.47筛选完成：{len(df)}只｜稳健优选排序｜经过防守韧性筛选{extra}')
        except Exception as exc:
    ...
    def build_backtest_tab(self):
        ...
        daily_cols=('日期','候选数','原始候选数','冷却跳过','TopN','T+1单日平均%','T+2单日平均%','T+3单日平均%','T+4单日平均%','T+5单日平均%','T+1累计平均%','T+2累计平均%','T+3累计平均%','T+4累计平均%','T+5累计平均%','T+1上涨率','T+2上涨率','T+3上涨率','T+4上涨率','T+5上涨率','T+5涨停率','状态')
        self.backtest_daily_tree=ttk.Treeview(daily_frame,columns=daily_cols,show='headings')
        dw={'日期':90,'候选数':65,'原始候选数':85,'冷却跳过':75,'TopN':55,'T+1单日平均%':88,'T+2单日平均%':88,'T+3单日平均%':88,'T+4单日平均%':88,'T+5单日平均%':88,'T+1累计平均%':88,'T+2累计平均%':88,'T+3累计平均%':88,'T+4累计平均%':88,'T+5累计平均%':88,'T+1上涨率':78,'T+2上涨率':78,'T+3上涨率':78,'T+4上涨率':78,'T+5上涨率':78,'T+5涨停率':78,'状态':90}
        ...
        result_cols=('推荐日','推荐排名','推荐度','代码','名称','推荐日收盘','T+1单日涨幅%','T+2单日涨幅%','T+3单日涨幅%','T+4单日涨幅%','T+5单日涨幅%','T+6单日涨幅%','T+1累计涨幅%','T+2累计涨幅%','T+3累计涨幅%','T+4累计涨幅%','T+5累计涨幅%','T+6累计涨幅%','累计校验最大差异%','T+1开盘买入','买入价','交易结果','持有交易日','卖出价','净收益%','第一止盈%','第二止盈%','软止损%','硬止损%','首止盈卖出仓位%','首次止盈日','最终清仓日','最高浮盈%','最大浮亏%','交易备注','原T+5收盘%')
    ...
    def _backtest_worker(self,db_path,start,end,topn,mode,rank_mode,params,meta,trade_target,trade_target2,soft_stop,trade_stop,partial_pct,trade_cost,cooldown_days):
        try:
            ...
            for di,day in enumerate(dates,1):
                ...
                if cdf.empty:
                    daily_rows.append({'日期':day,'候选数':0,'原始候选数':0,'冷却跳过':0,'TopN':0,'T+1上涨率':'','T+2上涨率':'','T+3上涨率':'','T+4上涨率':'','T+5上涨率':'','T+1平均收益%':'','T+2平均收益%':'','T+3平均收益%':'','T+4平均收益%':'','T+5平均收益%':'','T+1单日平均%':'','T+2单日平均%':'','T+3单日平均%':'','T+4单日平均%':'','T+5单日平均%':'','T+1累计平均%':'','T+2累计平均%':'','T+3累计平均%':'','T+4累计平均%':'','T+5累计平均%':'','T+5涨停率':'','状态':'无候选'}); continue
                ...
                if cooldown_days>0:
                    ...
                    skipped=0
                if cdf.empty:
                    daily_rows.append({'日期':day,'候选数':0,'原始候选数':original_candidate_count,'冷却跳过':skipped,'TopN':0,'T+1上涨率':'','T+2上涨率':'','T+3上涨率':'','T+4上涨率':'','T+5上涨率':'','T+1平均收益%':'','T+2平均收益%':'','T+3平均收益%':'','T+4平均收益%':'','T+5平均收益%':'','T+1单日平均%':'','T+2单日平均%':'','T+3单日平均%':'','T+4单日平均%':'','T+5单日平均%':'','T+1累计平均%':'','T+2累计平均%':'','T+3累计平均%':'','T+4累计平均%':'','T+5累计平均%':'','T+5涨停率':'','状态':'全部被冷却'}); continue
                cdf=apply_extreme_picks(cdf, params.get('extreme_pick_count', 5))
                if rank_mode=='极度推荐':
                    cdf=cdf.sort_values(['稳健优选分','稳定性分','极度推荐分','涨停基因分','推荐度'],ascending=[False,False,False,False,False],na_position='last').reset_index(drop=True)
                else:
