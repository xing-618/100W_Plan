...
# ============================================================
# A股短线研究工具 V1.46：MACD修复 + 防守韧性筛选 + 稳健优选排序
#
# V1.2 目标：
...
DEFAULT_PARAMS = {
    ...
    # V1.46：防守韧性层——弱市不再只做加分，而是进入硬筛选；主列表优先按稳健优选分排序。
    'extreme_pick_count': 5,
    # V1.46 极度推荐：对应个人看盘框架，而不是单一指标排名。
    ...
    # V1.46：防守韧性层。目标是筛出“市场转弱时相对不容易跟跌”的历史行为。
    # 注意：这是历史行为代理，不保证未来不跌。
...
def macd_stage_state(qfq_df, tangle_ratio=0.35):
    """V1.46：显式识别早期死叉修复、缠绕横盘、弱修复和前期大涨回撤。"""
    out = {
...
def turnover_rate_from_profile(raw_df, profile):
    """V1.46：用成交额/流通市值计算近似换手率，避免依赖额外接口。"""
    try:
...
def sector_context_map(raw_dict, qfq_dict, profiles):
    """V1.46：基于同日全市场股票画像+日线，给候选补充所属板块环境。"""
    groups = {}
...
def limit_up_gene_state(raw_df, threshold=9.8, lookback=LOOKBACK_TRADING_DAYS):
    """V1.46：把涨停基因从“涨停次数”升级为历史弹性、近期密度、距离最近涨停
    以及历史涨停后3个交易日的延续能力。
...
def _market_daily_series(raw_dict):
    """V1.46：构造目标日及以前的全市场日度中位涨跌、上涨占比等序列。"""
    rows = []
...
def calculate_extreme_score(raw_df, qfq_df, params, market_ctx=None, profile=None, sector_ctx=None, defensive=None):
    """V1.46：极度推荐在V1.46多因子基础上加入防守韧性，但不允许缺失数据凭空加分。"""
...
def screen_stock(code, name, qfq_df, raw_df, params, market_ctx=None, profile=None, sector_ctx=None):
    ...
    # V1.46：传统信号通过后，再做防守韧性层。弱市时门槛自动提高。
...
def screen_stocks(data_manager, params, screen_date):
    ...
    if not df.empty:
        # V1.46：主列表改为稳健优选分优先；传统推荐度保留作辅助参考。
...
class StockResearchGUI:
    ...
    def build_review_tab(self):
        ...
        columns = ('排名','极度推荐','稳健优选分','稳定性分','代码','名称','极度推荐分','推荐度','弱市跑赢率%','弱市不跌率%','弱市相对优势%','稳定性判断','启动评分','涨停基因分','MACD阶段','MACD距','MACD距变化2日%','绿柱连续缩短','修复质量','前期形态','最新价','今日涨跌%','MA5状态','RSI14','KDJ状态','量比','5/20量比','20日涨幅%','60日高点回撤%','近一年涨停次数','相对强度','市场状态','收盘交易状态')
        ...
        widths = {
            ...
            '20日涨幅%':84,'60日高点回撤%':100,'近一年涨停次数':94,'相对强度':78,'市场状态':110,'收盘交易状态':220
        }
        for c in columns:
    ...
    def refresh_market_profiles_thread(self):
        def worker():
            try:
                # V1.46：先走东方财富公开画像，不因为市值/行业刷新而强制安装AKShare。
    ...
    def _screen_worker(self, params, target):
        try:
            ...
            self.set_status(f'{target} V1.46筛选完成：{len(df)}只｜稳健优选排序｜经过防守韧性筛选{extra}')
        except Exception as exc:
