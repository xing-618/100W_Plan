...
# ============================================================
# A股短线研究工具 V1.43：BaoStock断线自愈 + 新浪兜底
#
# V1.2 目标：
...
DEFAULT_PARAMS = {
    ...
    # V1.43：二级优选层——保留全量候选池，独立输出极度推荐 Top 5。
    'extreme_pick_count': 5,
    'extreme_macd_weight': 30.0,
    'extreme_limit_gene_weight': 30.0,
    'extreme_volume_weight': 15.0,
    'extreme_price_timing_weight': 10.0,
    'extreme_market_weight': 10.0,
    'extreme_confirm_weight': 5.0,
}
...
def limit_up_gene_state(raw_df, threshold=9.8, lookback=LOOKBACK_TRADING_DAYS):
    """V1.43：把涨停基因从“涨停次数”升级为历史弹性、近期密度、距离最近涨停
    以及历史涨停后3个交易日的延续能力。
...
def calculate_extreme_score(raw_df, qfq_df, params, market_ctx=None):
    """V1.43：候选池之上的独立“极度推荐”评分，不改变候选池数量。"""
    ...
    rs = _relative_strength(raw_df, market_ctx or {})

    weights = {
        'macd': float(params.get('extreme_macd_weight', 30.0)),
        'gene': float(params.get('extreme_limit_gene_weight', 30.0)),
        'volume': float(params.get('extreme_volume_weight', 15.0)),
        'price': float(params.get('extreme_price_timing_weight', 10.0)),
        'market': float(params.get('extreme_market_weight', 10.0)),
        'confirm': float(params.get('extreme_confirm_weight', 5.0)),
    }
    total_w = max(1.0, sum(weights.values()))

    macd_part = float(np.clip(mr.get('score', 0.0), 0, 100))
    gene_part = float(np.clip(gene.get('score', 50.0), 0, 100))

    # 量能：前期缩量 + 当前温和放量更符合“启动前”状态。
    vol_part = 50.0
    ...
    if vol is not None and not pd.isna(vol):
        if 1.0 <= vol <= 1.6:
            ...
            vol_part -= 5
    vol_part = float(np.clip(vol_part, 0, 100))

    # 推荐日越安静越好；明显上涨直接降权，4%已由硬条件排除。
    pct = safe_float(ps.get('pct'))
    if pct is None or pd.isna(pct):
        price_part = 50.0
    elif -1.0 <= pct <= 1.0:
        price_part = 100.0
    ...
    if ret20 is not None and not pd.isna(ret20):
        if ret20 > 15:
            ...
            price_part -= 5
    if pos120 is not None and not pd.isna(pos120) and pos120 > 0.80:
        price_part -= 10
    price_part = float(np.clip(price_part, 0, 100))

    market_score = safe_float((market_ctx or {}).get('score'))
    if market_score is None:
        market_score = 50.0
    rs_score = safe_float(rs.get('score'))
    if rs_score is None:
        rs_score = 50.0
    market_part = float(np.clip(0.55 * market_score + 0.45 * rs_score, 0, 100))
    ...
    if k is not None and d is not None:
        if k > d:
            confirm += 12
        elif k < d and j is not None and j < 20:
            confirm += 2
        else:
            confirm -= 5
    confirm = float(np.clip(confirm, 0, 100))

    score = (
        weights['macd'] * macd_part +
        weights['gene'] * gene_part +
        weights['volume'] * vol_part +
        weights['price'] * price_part +
        weights['market'] * market_part +
        weights['confirm'] * confirm
    ) / total_w

    reasons = [mr.get('state', 'MACD未知'), gene.get('state', '涨停基因未知')]
    if gene.get('follow_samples', 0) >= 3:
        reasons.append(f"历史涨停后3日≥3%命中{gene.get('follow_hit3', 0)*100:.0f}%")
    if pct is not None and not pd.isna(pct):
        reasons.append(ps.get('state', '今日状态未知'))
    shrink_ok = con is not None and not pd.isna(con) and 0.55 <= con <= 0.90
    vol_ok = vol is not None and not pd.isna(vol) and 1.0 <= vol <= 1.6
    if shrink_ok:
        reasons.append('前期缩量结构较好')
    if vol_ok:
        reasons.append('当前量能温和放大')

    return {
        'score': round(float(np.clip(score, 0, 100)), 1),
        'reason': '；'.join(reasons[:6]),
        'gene': gene,
        'macd_part': round(macd_part, 1),
        'volume_part': round(vol_part, 1),
        'price_part': round(price_part, 1),
        'market_part': round(market_part, 1),
        'confirm_part': round(confirm, 1),
    }


def apply_extreme_picks(df, top_n=5):
    """V1.43：不减少候选池，只标记极度推荐 Top N。"""
    if df is None or df.empty:
        return df
    out = df.copy()
    out['极度推荐'] = ''
...
def calculate_recommend_score(raw_df, qfq_df, params):
    ...
    score = 0.0
    # 1. 历史强势基因：保留涨停次数，同时引入V1.43涨停基因质量。
    count_component = 12 * _clip_score(limits, params['min_limit_ups'], max(params['min_limit_ups'] + 8, 10))
    gene_component = 8 * (gene.get('score', 50.0) / 100.0)
    score += count_component + gene_component
    # 2. 趋势 15分
    score += trend_bonus
    # 3. 量能：放量但不爆量 15分
    if vol_ratio >= 1:
...
def cache_stock_profiles(db_path, df, snapshot_date=None):
    ...
    for _,r in df.iterrows():
        ...
        rows.append((snapshot_date,code,name,industry,theme,total,circ,shares,'AkShare-东方财富',dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
...
def enrich_profiles_before_screen(data_manager, screen_date, codes):
    ...
    if target == today_str() and ak is not None:
...
def build_result(code, name, raw_df, qfq_df, params, market_ctx=None, profile=None):
    price = float(raw_df.iloc[-1]['收盘'])
    ...
    extreme = calculate_extreme_score(raw_df, qfq_df, params, market_ctx)
    gene = extreme.get('gene', {})
    # V1.43：极度推荐独立计算，不替换原推荐度。
    ...
    return {
        ...
        '流通市值(亿)': round((profile or {}).get('circulating_market_cap_billion'), 2) if (profile or {}).get('circulating_market_cap_billion') is not None else '',
        '参考总市值(亿)': round((profile or {}).get('total_market_cap_billion'), 2) if (profile or {}).get('total_market_cap_billion') is not None else '',
        ...
        '市值数据源': (profile or {}).get('source',''),
        '市值快照日期': (profile or {}).get('snapshot_date',''),
...
def screen_stock(code, name, qfq_df, raw_df, params, market_ctx=None, profile=None):
    ...
    if rsi is not None and rsi >= 85 and k is not None and d is not None and k < d:
        return None
    return build_result(code, name, raw_df, qfq_df, params, market_ctx, profile)
...
def screen_stocks(data_manager, params, screen_date):
    ...
    for code,qfq_df in qfq.items():
        ...
        result=screen_stock(code,names.get(code,''),qfq_df,raw.get(code),local_params,market_ctx,profile)
        if result: results.append(result)
    df=pd.DataFrame(results)
    if not df.empty:
        # V1.43：主列表仍按推荐度排序；另标记独立的极度推荐 Top 5。
...
class StockResearchGUI:
    ...
    def build_review_tab(self):
        ...
        columns = ('排名','极度推荐','极度推荐分','代码','名称','推荐度','涨停基因分','启动评分','基础策略分','严格匹配度','最新价','今日涨跌%','今日状态','激进买点','保守买点','激进止损','保守止损','激进止盈','保守止盈','激进周期','保守周期','RSI14','KDJ状态','市场评分','市场状态','相对强度','相对大盘5日%','所属行业','主题标签','流通市值(亿)','参考总市值(亿)','市值状态','市值数据源','筹码结构','收盘交易状态','严格评级','关注理由')
        ...
        widths = {
            '排名':50,'极度推荐':78,'极度推荐分':88,'代码':72,'名称':95,'推荐度':72,'涨停基因分':82,'启动评分':80,'基础策略分':82,'严格匹配度':82,'最新价':75,'今日涨跌%':82,'今日状态':125,'RSI14':72,'KDJ状态':90,
    ...
    def refresh_market_profiles_thread(self):
        def worker():
            try:
                self.set_status('正在刷新行业/市值画像（AkShare）……')
                ok,msg=ensure_akshare_installed(quiet=True)
                if not ok:
                    self.ui(messagebox.showwarning,'行业/市值数据',msg); return
                df=fetch_current_stock_profiles()
                if df.empty:
                    self.set_status('行业/市值刷新失败：数据源为空'); return
            ...
            for idx,code in enumerate(qfq.keys(),1):
                ...
                if params.get('market_cap_filter_enabled',False):
                    ...
                    if not same_day or cap is None:
                        ...
                        lo=float(params.get('min_market_cap',0) or 0); hi=float(params.get('max_market_cap',999999) or 999999)
                        if lo<=cap<=hi:
                            result=screen_stock(code,names.get(code,''),qfq.get(code),raw.get(code),local_params,market_ctx,profile)
                        else:
                            cap_filtered+=1
                else:
                    result=screen_stock(code,names.get(code,''),qfq.get(code),raw.get(code),local_params,market_ctx,profile)
    ...
    def display_review_results(self):
        ...
        for i,(_,r) in enumerate(top5.iterrows(),1):
            top_items.append(f"{i}.{r['代码']} {r['名称']}({float(r.get('极度推荐分',0)):.1f}|涨停基因{float(r.get('涨停基因分',0)):.1f})")
    ...
    def show_review_detail(self,row):
        ...
        for title,keylist in [
            ...
            ('核心指标',['最新价','今日涨跌%','今日状态','极度推荐','极度推荐分','极度推荐理由','推荐度','涨停基因分','涨停基因状态','涨停基因说明','启动评分','基础策略分','严格匹配度','RSI14','KDJ状态','行业/主题','流通市值(亿)','参考总市值(亿)','市值状态','市场评分','市场状态','相对强度','相对大盘5日%','MACD零轴','DIF','DEA','MACD柱','近一年涨停次数','量比','5/20量比','20日涨幅%','近10日最大涨幅%']),
    ...
    def build_backtest_tab(self):
        ...
        pick_cols=('日期','排名','极度推荐排名','极度推荐分','涨停基因分','代码','名称','推荐度','启动评分','基础策略分','严格匹配度','最新价','激进买点','保守买点','激进止损','保守止损','激进止盈','保守止盈','激进周期','保守周期','市场评分','市场状态','相对强度','行业/主题','流通市值(亿)','参考总市值(亿)','筹码结构','关注理由')
        ...
        try:
            ...
            for di,day in enumerate(dates,1):
                ...
                for _,rec in picks.iterrows():
                    code=normalize_code(rec['代码']); trade=self._simulate_trade(rmap.get(code,pd.DataFrame()),day,trade_target,trade_target2,soft_stop,trade_stop,partial_pct,trade_cost); pick_rows.append({'日期':day,'排名':rec['排名'],'极度推荐排名':rec.get('极度推荐排名',''),'极度推荐分':rec.get('极度推荐分',''),'涨停基因分':rec.get('涨停基因分',''),'代码':code,'名称':rec.get('名称',''),'推荐度':rec.get('推荐度',''),'严格匹配度':rec.get('严格匹配度',''),'最新价':rec.get('最新价',''),'筹码结构':rec.get('筹码结构',''),'行业/主题':rec.get('行业/主题',''),'流通市值(亿)':rec.get('流通市值(亿)',''),'参考总市值(亿)':rec.get('参考总市值(亿)',''),'收盘交易状态':rec.get('收盘交易状态',''),'激进买点':rec.get('激进买点',''),'保守买点':rec.get('保守买点',''),'激进止损':rec.get('激进止损',''),'保守止损':rec.get('保守止损',''),'激进止盈':rec.get('激进止盈',''),'保守止盈':rec.get('保守止盈',''),'关注理由':rec.get('关注理由','')}); base=self._future_metrics(rmap.get(code,pd.DataFrame()),day,{'推荐日':day,'推荐排名':rec['排名'],'极度推荐排名':rec.get('极度推荐排名',''),'极度推荐分':rec.get('极度推荐分',''),'涨停基因分':rec.get('涨停基因分',''),'推荐度':rec.get('推荐度',''),'代码':code,'名称':rec.get('名称','')});
